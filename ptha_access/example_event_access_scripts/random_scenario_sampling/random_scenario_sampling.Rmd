...
```
Another important variable is `rate_annual`. In general this varies for each
scenario, and is equal to the scenario conditional probability (conditional on
the occurrence of an earthquake with the same magnitude) multiplied by the
logic-tree-mean-rate of scenarios with that magnitude (events/year). 
