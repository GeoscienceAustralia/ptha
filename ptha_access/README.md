...
from these files. Here we illustrate that for the variable-shear-modulus case:

```r
# Sequence of magnitude values
mws = seq(7.15, 9.85, by=0.1)
# Mean exceedance rate curve (assuming variable shear modulus)
mean_rates = sapply(mws, f<-function(x){
    sum(puysegur$events$variable_mu_rate_annual * (puysegur$events$variable_mu_Mw > x))
    })
# 2.5% exceedance rate curve (assuming variable shear modulus)
lower_ci_rates = sapply(mws, f<-function(x){
    sum(puysegur$events$variable_mu_rate_annual_lower_ci * (puysegur$events$variable_mu_Mw > x))
    })
# 97.5% exceedance rate curve (assuming variable shear modulus)
upper_ci_rates = sapply(mws, f<-function(x){
    sum(puysegur$events$variable_mu_rate_annual_upper_ci * (puysegur$events$variable_mu_Mw > x))
    })

# Make a plot
plot(mws, mean_rates, t='l', log='y', lwd=2, 
    xlim=c(7., 9), ylim=c(1.0e-06,1), 
    main='Mw-exceedance-rate curve (variable shear-modulus)', 
...
```r
points(mws, upper_ci_rates, t='l', col='red')
points(mws, lower_ci_rates, t='l', col='red')
grid(col='orange')
...
It is important to understand that at the individual scenario level, there is
no such ordering constraint. For example, the `rate_annual_16pc` value is
...
applies to the cumulative sum of the individual scenario rates, but not the rates
themselves.
