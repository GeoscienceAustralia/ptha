...
## Obtaining tsunami max-stage exceedance-rates at sites around Australia

The tsunami 'max-stage' is the maximum water-level that a particular tsunami
...
mean-sea-level (MSL=0), so the 'max-stage' is equivalent to the maximum
elevation of the tsunami wave above MSL. 

The max-stage exceedance-rates describe 'how often' tsunami events occur with
max-stage above a particular threshold value. For example, you could ask how
often max-stages above 0.5m (or 2.0m) occur, in terms of the average number of
...
The simplest way to examine the tsunami max-stage exceedance-rates in
...
* multiple columns with names like `STAGE_XXXX` where XXXX is a number, and 1/XXXX is the exceedance-rate. These values corresponds to the tsunami max-stage which has mean exceedance-rate = 1/XXXX. For example, the column `STAGE_100` gives the tsunami max-stage that is exceeded once every 100 years on average, according to the mean of all the rate models in our logic-tree.
...
max-stage for a given exceedance-rate. This is mainly due to large uncertainty
...
The max-stage exceedance-rates vary from site to site, depending on exposure
...
which can be found by examining the max-stage vs exceedance-rate datasets (csv
