...
for modelling the early part of the tsunami time-series, even at sites far from
the earthquake-source, because the early arrivals are generally dominated by
waves that have predominantly travelled through the deep ocean where friction
is small. However late arrival are more likely to represent waves that
have reflected off coastlines or travelled through shallow waters, where
friction is greater. *Hence frictionless simulations are likely to overestimate
the size of late-arriving waves in any location.* To work around this it is
preferable to simulate the tsunami from source using a model that includes
global-scale friction, especially if the most significant waves occur 8 or more
hours after the tsunami arrives at your site of interest. 
