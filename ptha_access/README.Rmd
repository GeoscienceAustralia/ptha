...
a csv file containing the data here: <http://dapds00.nci.org.au/thredds/fileServer/fj6/PTHA/AustPTHA_1/EVENT_RATES/tsunami_stages_at_fixed_return_periods.csv>. This csv file contains columns:
    ...
    * multiple columns with names like `variable_mu_stochastic_slip_stage_XXXX` where XXXX is a number, and 1/XXXX is the exceedance rate. 

        These values corresponds to the tsunami peak-stage which has mean exceedance rate = 1/XXXX. For example, the column `variable_mu_stochastic_slip_stage_100` gives the tsunami peak-stage that is exceeded once every 100 years on average, according to the mean of all the rate models in our logic-tree.
   
    * multiple columns with names like `variable_mu_stochastic_slip_stage_upper_ci_XXXX`. 
    ...
    * multiple columns with names like `variable_mu_stochastic_slip_stage_lower_ci_XXXX`. 
...
The unusual variable name (`variable_mu_stochastic_slip_stage_`) is used
because the scenarios are constructed assuming depth-varying shear modulus on
subduction zones (`variable_mu`), and also treat earthquake slip as random and
spatially heterogeneous (`stochastic_slip`). 

Similar data is available in a shapefile, which can be downloaded here:
<http://dap.nci.org.au/thredds/remoteCatalogService?catalog=http://dapds00.nci.org.au/thredds/catalog/fj6/PTHA/AustPTHA_1/EVENT_RATES/tsunami_stages_at_fixed_return_periods/catalog.xml>
However, due to the 10 character limit on shapefile attribute names, the attributes are named as:
    * `lon`, `lat` giving the location in longitude/latitude (degrees). 
    * `elev` is the bathymetry at the station (negative = below MSL)
    * `gaugeID` is a real station ID
    * `S_XXXX' is the same as `variable_mu_stochastic_slip_stage_XXXX` described above
    * `STG_XXXX' is the same as `variable_mu_stochastic_slip_stage_XXXX` described above
    * `STGu_XXXX' is the same as `variable_mu_stochastic_slip_stage_upper_ci_XXXX` described above
    * `STGl_XXXX' is the same as `variable_mu_stochastic_slip_stage_lower_ci_XXXX` described above

At most locations you will find there is high uncertainty in the peak tsunami
size at various exceedance rates. This is mainly caused by the fact that at
most locations, we are very uncertain of how often large subduction earthquakes
occur. A more detailed discussion of these topics can be found in the
Australian Tsunami Hazard Modelling Guidelines, which are available here:
...
If you have R installed, then the latter can be obtained by typing the
following in R:
...
You also need to install the R package `ncdf4`. Unfortunately this needs to be
built with a reasonably recent version of netcdf-c, due to a bug in netcdf
versions prior to 4.6.1 (released in early 2018) that prevented correct remote
reading of long character strings. At the time of writing (mid 2018), most
users will have to install netcdf from source to access such a recent version.
Source-code for a recent release of netcdf-c can be obtained from the netcdf
github page: <https://github.com/Unidata/netcdf-c/releases>. You then need to
follow their instructions to get it installed.

Supposing you succeed in getting netcdf installed, next you need to install R's
`ncdf4` package, and specifically tell it to use the newly installed netcdf.
This can be done by downloading the ncdf4 sources here <https://cran.r-project.org/web/packages/ncdf4/index.html>,
