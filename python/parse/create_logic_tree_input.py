...
for line in f_in.readlines():
    ...
    if row[10] != '':
        sparebs_pref[row[0]]=float(row[10].rstrip('\r\n'))
        sparebs_min[row[0]]=float(row[10].rstrip('\r\n')) - 0.1
        sparebs_max[row[0]]=float(row[10].rstrip('\r\n')) + 0.1
print mapping
f_in.close()
...
for line in f_in.readlines():
    ...
    b_value_pref = float(row[7])
    gem_bs_pref[name]=b_value_pref
    b_value_min = float(row[8])
    gem_bs_min[name]=b_value_min
    b_value_max = float(row[9].rstrip('\r\n'))
    gem_bs_max[name]=b_value_max
...
for key in mapping.keys():
    ...
    if mapping[key] == 'NULL':
        ...
        b_values_pref[key] = float(sparebs_pref[key])
        b_values_min[key] = float(sparebs_min[key])
        b_values_max[key] = float(sparebs_max[key])
        ...
        try:
            ...
            b_value_pref = gem_bs_pref[gem_name]
            b_value_min = gem_bs_min[gem_name]
            b_value_max = gem_bs_max[gem_name]
        ...
        b_values_pref[key] = b_value_pref
        b_values_min[key] = b_value_min
        b_values_max[key] = b_value_max
        mmaxs[key] = mmax
...
for source_zone in source_zones:    
    if source_zone.startswith('north_africa'):
        if source_zone == 'north_african':
            pass
        else:
            continue
...
#max_mag
    ...
    lines[14] = '         3             Number of b-values'
    lines[15] = '         %.2f %.2f %.2f b-values' % (b_values_pref[source_zone], \
                                                         b_values_min[source_zone],\
                                                         b_values_max[source_zone])
    lines[16] = '         0.6 0.2 0.2   Weights for b-values'
...
#    lines[23] = '         0.5 1.0 0.25  Delta_m1, delta_m2, for char mag rec model'
    ...
    lines[28] = '         ' + mmaxs_pref[source_zone] + ' ' + mmaxs[source_zone] + ' ' + mmaxs_min[source_zone] + \
        '          Maximum magnitudes'
    lines[29] = '         0.6 0.3 0.1          Weights for maximum mags'
