...
#define CORIOLIS
    ...
    !    real(dp) :: lw(2), lower_left(2), dx(2), dx_refinement_factor
    !    integer(ip) :: timestepping_refinement_factor, nx(2)
    !    character(len=charlen) :: timestepping_method 
    !end type

    !
    ! Original aim was to have a type to only hold domain metadata here
    ! The main 'domain_type' would then extend this
    !
    ...
    !
    ! Currently implemented alternative is to put it all in the domain [see
    ! commented lines]
    !
    !type :: domain_metadata_type
    type :: domain_type
...
!#include "domain_routines_vectorized_include.f90"
    ...
    subroutine allocate_quantities(domain, global_lw, global_nx, global_ll, create_output_files,&
        ...
        ! promote openmp memory affinity (based on the first-touched) idea,
        ! where memory will be local to the thread that first touched it
        !
        ...
        if(j > 2) then
            if(reuse_gradients) then
                ...
                theta_wd_neg_B = limiter_coef4*( &
                    ...
                    - limiter_coef1)
                !if(theta_wd_neg_B < ONE_dp) error stop('It goes less than one')
                    ...
                    do i = 1, domain%nx(1)
                        if(domain%depth(i,j) == masscon_error_neg_depth) then
                            ...
                            if(allocated(domain%nesting%priority_domain_index)) then
                                write(domain%logfile_unit,*) 'priority image and index: ', &
    ...
    subroutine compute_fluxes(domain, max_dt_out)
        ...
        real(dp):: denom, inv_denom, max_speed, max_dt, dx_cfl_inv(2)
        ...
        real(dp), parameter :: EPS_gs = sqrt(gravity * minimum_allowed_depth) 
            ...
            do i = 2, nx
                ...
                if(reduced_momentum_diffusion) then
                    ...
                    fr2 = advection_beta * sqrt(max(0.001_dp, min(1.0_dp, &
                        0.5_dp*(v_neg**2 + v_pos**2 + u_neg**2 + u_pos**2)/(gs_neg**2 + gs_pos**2 + 1.0e-10_dp))))
                else
                    fr2 = 1.0_dp
                end if
                ...
                if(flux_conservative_pressure) then
                    ...
                    domain%explicit_source_VH_j_minus_1(i,j) = &
                        -domain%area_cell_y(j-1) * gravity * depth_neg_c * stage_neg / domain%distance_left_edge(i)

                    domain%explicit_source(i,j,VH) = domain%explicit_source(i,j,VH) + &
                        domain%area_cell_y(j) * gravity * depth_pos_c * stage_pos / domain%distance_left_edge(i)
                end if
        ...
        !
                ...
                if(flux_conservative_pressure) then
                    ...
                    domain%explicit_source(i-1,j,UH) = domain%explicit_source(i-1,j,UH) - &
                        domain%area_cell_y(j) * gravity * depth_neg_c * stage_neg / dxb 

                    domain%explicit_source(i,j,UH) = domain%explicit_source(i,j,UH) + &
                        domain%area_cell_y(j) * gravity * depth_pos_c * stage_pos / dxb
    ...
    subroutine one_rk2_step(domain, timestep)
        ...
        !
        ! rk2 involves taking 2 euler steps, then dividing by 2.
        !
