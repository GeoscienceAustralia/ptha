...
- `-DLEGACY_CORIOLIS_PARAMETER` Previously SWALS use a value for the earth's angular frequency assuming one rotation per day. This is slightly incorrect (it's about 0.3% to large) and so was updated to a more accurate value, but the older value can be invoked by passing this flag.
