...
- `-DLEGACY_CORIOLIS_PARAMETER` Previously SWALS use a value for the earth's angular frequency assuming one rotation per day. Although this matches several other tsunami codes, it is slightly incorrect (about 0.3% too large, because the earth rotates slightly less than once per day) so was updated. This flag makes SWALS use the slightly-too-large value. In principle this might help to compare with a previous run, or another model that uses the same treatment, although in practice I haven't seen a case where it matters. 
