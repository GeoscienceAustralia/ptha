...
If dispersion is used on any domain (`any(md%domains(:)%use_dispersion)`) then SWALS also solves the dispersive terms using an implicit method. This requires much more communication and computation. In nested grid models the implicit method may need halos that contain more cells than SWALS provides by default, particularly on domains that only take 1 timestep for every global timestep (one can increase the halo thickness on particular domains by setting the integer `md%domains(j)%minimum_nesting_layer_thickness`) . The dispersive model is mainly valid in the linear regime, and should be turned off when nonlinearities become too great. SWALS allows the user to specify two depths (`domain%ds%td1` $>$ `domain%ds%td2`) so that dispersion is linearly tapered to zero as $h_0$ varies between `td1` and `td2`. If using finite volume methods with dispersion, it is suggested to use `midpoint` which is more accurate than the other finite volume solvers because the dispersive terms are included in both the predictor and the main timestep (for the other solvers this is harder to achieve in conjunction with nesting).
