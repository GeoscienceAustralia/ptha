...
* To solve the resulting implicit equations, we compute part of the dispersive terms involving the current time level, then then take a shallow water step, then guess the solution at the next time level including dispersion (using quadratic-in-time extrapolation from the solution each global timestep). We then iteratively update the guess with implicit solves of the dispersive terms at the next time level, applied sequentially in the x-and-y directions, which leads to a set of tridiagonal matrix solves. After two iterations of these sequential x-and-y implicit solves, we communicate halos with neighbouring nested domains that have advanced to the same sub-time-step, and then apply a further two iterations (so the updated halos can influence the solution) and communicate again. In practice this seems to be enough. 
    * If needed, the number of iterations in between halo exchanges can be specified with `md%domains(j)%ds%tridiagonal_inner_iter` (default 2), and the number of halo exchanges per sub-time-step with `md%dispersive_outer_iterations_count` (default 2).
* This requires much more communication and computation than non-dispersive solves. In nested grid models the implicit method may need halos that contain more cells than SWALS provides by default, particularly on domains that only take 1 timestep for every global timestep. To address this one can increase the halo thickness by setting the integer `md%domains(j)%minimum_nesting_layer_thickness`.
* The dispersive model is mainly valid in the linear regime, and should be turned off when nonlinearities become too great. SWALS allows the user to specify two depths (`domain%ds%td1` $>$ `domain%ds%td2`) so that dispersion is linearly tapered to zero as $h_0$ varies between `td1` and `td2`. 
* If using finite volume methods with dispersion, it is suggested to use `midpoint` which is more accurate than the other finite volume solvers because the dispersive terms are included in both the predictor and the main timestep (for the other solvers this is harder to achieve in conjunction with nesting).
