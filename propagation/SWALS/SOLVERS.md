...
The turbulent diffusion term $\mathbf{\Psi}$ is by default set to zero. But a rudimentary eddy-viscosity turbulence model can be used *for the finite volume schemes only* by specifying `md%domains(j)%use_eddy_viscosity=.true.`. Then $\mathbf{\Psi} = -\nabla \cdot (\epsilon h (\nabla \otimes \mathbf{u}))$. Here $\epsilon$ is the eddy viscosity and $\nabla \otimes \mathbf{u}$ is a 2x2 matrix with columns $\nabla u$, $\nabla v$ respectively. The eddy viscosity can be controlled and adjusting the values of `md%domains(j)%eddy_visc_constants(1:2)`. 
