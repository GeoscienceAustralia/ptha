...
* Solution method (for all numerical schemes)
    ...
    * To solve the resulting implicit equations, imagine splitting the multidomain global timestep $dt$ into a $N_{t}$ substeps of size $\delta t$, where $\delta t$ is the smallest timestep required by any *dispersive* domain. Say $dt = N_{t}\delta t$, so the substeps can be indexed as $1, 2, 3, \ldots, N_{t}$. Domain $j$ has its own timestep (`alpha_j`$\delta t$), usually determined by its CFL condition. SWALS requires dispersive domains to have `alpha_j` being an integer that EXACTLY divides $N_{t}$ (e.g. if $N_{t} = 6$ then dispersive domains could have `alpha_j = 1, 2, 3, 6`). Non-dispersive domains have the same constraint, unless they take a timestep smaller than $\delta t$ (i.e. in which case we could have `alpha_j = 1./2, 1./3, 1./4, ...`).  
    * For each substep $s = 1, 2, \ldots, N_{t}$, we loop over domains
        * Skip domains where `alpha_j >= 1` and `mod(s-1, alpha_j) != 0`
        * For those that remain
            ...
            * Implicit solve
                * Dispersive domains receive halos from domains that have advanced to the same time. 
                ...
                * Dispersive domains iteratively update it with implicit solves applied sequentially in the x-and-y directions (tridiagonal solves). These inner iterations are repeated once by default (corresponding to `md%domains(j)%tridiagonal_inner_iter=2`).
                * Dispersive domains communicate halow data, but do not immediately update the solution
                * We then repeat the above sequence of implicit solve steps once by default (`md%dispersive_outer_iterations_count = 2` by default), except that now we onl receive halos from domains that have advanced to the same time AND have the same grid size. Unequal grid sizes would always disagree on the solution due to interpolation, leading to worse numerical behaviour if they are included. 
...
* Models using dispersion thus require much more communication and computation (about 3x compute for `midpoint`, vs non-dispersive models). In nested grid models, the implicit method may need halos to be thicker than SWALS provides by default, particularly on domains that only take 1 timestep for every global timestep (which are less likely to have thick halos by default). To address this one can increase the halo thickness by setting the integer `md%domains(j)%minimum_nesting_layer_thickness` (e.g. a value of 30 is used on the coarsest grids for some test problems).
    * Apply flux-correction after the final substep, and communicate once more. At this point all domains will have advanced one global timestep $dt = N_{t}\delta t$.
