...
* The time-derivatives are represented as the difference between terms at the current time level and the next time level. 
    * To solve them we compute the terms involving the current time level, then then take a shallow water step, then guess the solution including dispersion (using quadratic-in-time extrapolation from the solution each global timestep), then iteratively update the guess with implicit solves of terms at the next time level applied sequentially in the x-and-y directions. After two iterations we communicate the solution with neighbouring nested domains that have advanced to the same sub-step, and then apply a further two iterations. 
* This requires much more communication and computation than non-dispersive solves. In nested grid models the implicit method may need halos that contain more cells than SWALS provides by default, particularly on domains that only take 1 timestep for every global timestep (one can increase the halo thickness on particular domains by setting the integer `md%domains(j)%minimum_nesting_layer_thickness`).
