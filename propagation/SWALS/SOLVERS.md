...
The solvers support use of either Cartesian or Spherical coordinates by specifying the appropriate [compile time option](README.md#advanced-compilation). By default Cartesian coordinates are used. To use spherical coordinates we set the preprocessor varable `-DSPHERICAL` when compiling. Many examples are provided in the test-problems; for instance [this makefile](examples/nthmp/Hilo_Tohoku_tsunami/) uses spherical coordinates (and unrelated code timing) by defining the variable `SWALS_PREPROCESSOR_FLAGS := -DTIMER -DSPHERICAL`. If the variable `-DSPHERICAL` were not set then Cartesian coordinates would be used.
