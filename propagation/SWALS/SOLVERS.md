    ...
    * For each substep $s = 1, 2, \ldots, N_{t}$, we loop over domains
        ...
        * For those that remain
            ...
            * Implicit solve
                ...
                * We then repeat the above sequence of implicit solve steps once by default (`md%dispersive_outer_iterations_count = 2`), except that these *outer iterations* only receive halos from domains that have advanced to the same time AND have the same grid size. Unequal grid sizes would always disagree on the solution due to interpolation, leading to worse numerical behaviour if they are included on subsequent iterations. 
