...
* The time-derivatives are represented as the difference between terms at the current time level and the next time level; to solve them we use quadratic-in-time extrapolation to predict the solution at the next time level, and then iteratively update the solution with implicit solves applied sequentially in the x-and-y directions. After two iterations we communicate the solution with neighbouring nested domains and then apply a further two iterations. 
