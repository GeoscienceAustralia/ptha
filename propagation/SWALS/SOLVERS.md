...
In Cartesian coordinates SWALS does not provide a Coriolis term $(\vec{\Omega_s} = 0)$. In Spherical coordinates the Coriolis force is $\vec{\Omega_s} = 2 \sin(\phi_r) \gamma_r (-vh, uh)$, where $\gamma_r = 7.292115 \times 10^{-5}$ gives the earth angular frequency in radians per second. There is no Coriolis force in the CLIFFS solver. If using spherical coordinates with any other solver, it is possible to turn off the Coriolis force with the preprocessor variable `-DNOCORIOLIS`. 

The turbulent diffusion term $\vec{\Psi}$ is by default set to zero. A rudimentary eddy-viscosity turbulence model can be used *for the finite volume schemes only* by specifying `md%domains(j)%use_eddy_viscosity=.true.`. Then $\vec{\Psi} = -\nabla \cdot (\epsilon h (\nabla \otimes \vec{u}))$. Here $\epsilon$ is the eddy viscosity and $\nabla \otimes \vec{u}$ is a 2x2 matrix with columns $\nabla u$, $\nabla v$ respectively. The eddy viscosity can be controlled by adjusting the values of `md%domains(j)%eddy_visc_constants(1:2)`. 
...
* In spherical coordinates, SWALS neglects some terms in the expression for $\vec{\Psi}$ that are second order (except near the poles) related to the basis vectors changing direction from site to site.
...
By default the dispersive term $\vec{\Upsilon}=0$. However if `md%domains(j)%use_dispersion = .true.` then dispersive terms are included on domain `j`. They have the form $\vec{\Upsilon} = -\frac{h_0^2}{3} \nabla (\nabla \cdot \frac{\partial \vec{q}}{\partial t})$ which is widely used in the tsunami literature (e.g. [Saito et al., 2011](http://dx.doi.org/10.1029/2011GL049089), [Baba et al., 2015](http://dx.doi.org/10.1007/s00024-015-1049-2), and many others). Here $h_0$ is the depth below `md%domains(j)%msl_linear`, so when using dispersion it is critical to set the latter to a sensible value. In practice we taper the dispersive terms to zero linearly between two user-specified values of $h_0$ (below). There is a preprocessor option (`-DDISPERSIVE_PEREGRINE_IN_FLUX_FORM`) that modifies the dispersive terms to $\vec{\Upsilon}=-\frac{h_0^2}{2} \nabla (\nabla \cdot \frac{\partial \vec{q}}{\partial t}) + \frac{h_0^3}{6} \nabla (\nabla \cdot \frac{\partial \vec{q/h_0}}{\partial t})$ . In the linear regime the latter model is equivalent to the dispersive model of [Peregrine (1967)](https://doi.org/10.1017/S0022112067002605) although in flux form rather than the velocity form presented by Peregrine ( the difference matters in the nonlinear regime, e.g. [Filippini et al. 2015](http://dx.doi.org/10.1016/j.coastaleng.2015.02.003)). 
...
* If any domain uses dispersion, SWALS must use a different subroutine to advance the solution on all domains (whether or not they use dispersion). This requires substantially more communication and computation, places more restrictions on the allowed timestepping, and tends to reduce the stability of nesting 
    * Factor-of-3 spatial refinement seem reasonably stable with dispersion, as does factor-of-2 (although the latter is not often used). Larger ratios seem prone to long-time instability when dispersion is used.
    * If domains with the shortest timesteps do not use dispersion, then the communication burden can be significantly reduced. A typical situation like this occurs when dispersion matters for the global domain, but not for the highest resolution nearshore domains.
...
* Compared to models without dispersion, models using dispersion thus require much more communication (factor of $3N_t + 1$) and computation (about factor of 3 for `midpoint`). In nested grid models, the implicit method may need halos to be thicker than SWALS provides by default, particularly on domains that only take 1 timestep for every global timestep (which are less likely to have thick halos by default). To address this one can increase the halo thickness by setting the integer `md%domains(j)%minimum_nesting_layer_thickness` (e.g. a value of 30 is used on the coarsest grids for some test problems).
...
One should not try to specify nonlinear friction terms with the linear solvers. However, the non-standard linear friction model can still be used. 
