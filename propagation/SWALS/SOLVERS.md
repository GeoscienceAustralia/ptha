...
The friction slope term by default using Manning friction $\mathbf{S_f} = n^2\mathbf{u} |\mathbf{u}| h^{-4/3}$, with Manning coefficient $n$. But it can also use Chezy friction, which can be written in the same form using a different power of the depth ($\mathbf{S_f} = n^2\mathbf{u} |\mathbf{u}| h^{-1}$), where $n^{-2}$ is an unusual way of writing the Chezy coefficient. A non-standard linear friction term is also available (as discussed below). 
