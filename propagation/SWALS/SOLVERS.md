...
By default the dispersive term $\mathbf{\Upsilon}$ is zero. However if the user sets `md%domains(j)%use_dispersion = .true.` then dispersive terms are included on domain `j`. This causes SWALS to use a different subroutine to advance the solution - this needs substantially more communication and computation, and suffers more restrictions on timestepping and nesting stability. The dispersive terms have the form:

$$ -\frac{h_0^2}{3} \nabla (\nabla \cdot \frac{\partial \mathbf{q}}{\partial t}) $$

which is widely used in the tsunami literature. There is a preprocessor option (`-DDISPERSIVE_PEREGRINE_IN_FLUX_FORM`) that modifies this to 
