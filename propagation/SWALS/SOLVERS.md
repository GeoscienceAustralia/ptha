...
By default the dispersive term $\mathbf{\Upsilon}$ is zero. However if the user sets `md%domains(j)%use_dispersion = .true.` then dispersive terms are included on domain `j`. This causes SWALS to use a different subroutine to advance the solution - this needs substantially more communication and computation, and suffers more restrictions on timestepping and nesting stability. By default the dispersive terms have the form
$$ -\frac{h_0^2}{3} \nabla (\nabla \cdot \frac{\partial \mathbf{q}}{\partial t}) $$
which is widely used in the tsunami literature. Using a preprocessor option, this can be modified to
$$ -\frac{h_0^2}{2} \nabla (\nabla \cdot \frac{\partial \mathbf{q}}{\partial t}) + \frac{h_0^3}{6} \nabla (\nabla \cdot \frac{\partial \mathbf{q/h_0}}{\partial t})$$
which is equivalent (in the linear regime) to the dispersive model of [Peregrine (1967)](https://doi.org/10.1017/S0022112067002605) although in flux form rather than the velocity form presented by Peregrine ( which has an effect in the nonlinear regime, e.g. [Filippini et al. 2015](http://dx.doi.org/10.1016/j.coastaleng.2015.02.003)).
