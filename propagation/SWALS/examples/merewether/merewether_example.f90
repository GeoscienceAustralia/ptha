module local_routines
    ...
    use file_io_mod, only: read_character_file
    ...
    subroutine set_initial_conditions_merewether(domain)
        ...
        ! Prevent mass leaking by using walls for south, west boundaries
        domain%U(:, 1:2, ELV) = 100.0_dp
        domain%U(1:2, :, ELV) = 100.0_dp
        ! Flat boundary on the east and north so that the transmissive BC is well-behaved
        ...
        ! Set friction
        domain%manning_squared = friction_other * friction_other ! Background value (generic urban roughness)
        call road_pvt%setup([character(len=charlen)::'Road/RoadPolygon.csv'], [friction_road*friction_road]) ! Low road roughness
        call road_pvt%burn_into_grid(domain%manning_squared, domain%lower_left, domain%lower_left+domain%lw)

        ! Find indices inside the rainfall forcing region.
        allocate(rainfall_region_indices)
        allocate(rainfall_region_indices%i2(domain%nx(2)))
        num_input_discharge_cells = 0
        do j = 1, domain%nx(2)
            ! Cell indices on this row within the rain circle
            call which( (domain%x - rain_centre(1))**2 + (domain%y(j) - rain_centre(2))**2 < rain_radius**2, &
                rainfall_region_indices%i2(j)%i1)

            ! To cross-check conservation, count input discharge cells (allows separate mass balance)
        ...
        domain%forcing_context_cptr = c_loc(rainfall_region_indices) ! Data used in domain%forcing_subroutine
        call domain%store_forcing() ! Ensure the above forcings are stored (even if we append other forcing terms)
