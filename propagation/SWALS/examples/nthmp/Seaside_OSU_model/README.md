...
Figure 4 compares the model and observations at gauges WG3 and WG4. They are in general agreement once the model is time-shifted to the left by 0.7s, which previous studies also found to be needed due to a problem with the forcing (Macais et al., 2020). For this problem we do not expect perfect agreement as the flow is physically affected by non-hydrostatic physics, which is not represented with the nonlinear shallow water equations.
