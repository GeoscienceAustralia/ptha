...
module local_routines 
    ...
    subroutine set_initial_conditions(domain)
        ...
        ! Setup elevation
        !
        call elevation_data%initialise([character(len=charlen) :: &
            '../../nthmp/Conical_shelf_lab/bathy/bathy_with_cone.tif'])
...
program Conical_shelf
    ...
    md%domains(1)%timestepping_method = 'midpoint' !default_nonlinear_timestepping_method
    ...
    if(md%domains(1)%use_dispersion) then
        ! Turn off dispersion between these depths
        md%domains(1)%ds%td1 = 0.3_dp
        md%domains(1)%ds%td2 = 0.1_dp
    end if
    ...
    ! Add point gauges
    call md%set_point_gauges_from_csv('../../nthmp/Conical_shelf_lab/point_gauge_locations.csv', skip_header=1_ip)
