...
module local_routines 
    ...
    character(len=charlen) :: forcing_type = ''
    ...
    type :: boundary_information_type
        ...
        character(len=charlen):: wavemaker_file = &
            '../../nthmp/test_repository/BP06-FrankG-Solitary_wave_on_a_conical_island/fdbk2abc.txt'
        ...
        character(len=charlen) :: gauge_vel_file(3) = [ &
              '../../nthmp/BP06/gauge_forcing_nonlinear_case1.csv',&
              '../../nthmp/BP06/gauge_forcing_nonlinear_case2.csv',&
              '../../nthmp/BP06/gauge_forcing_nonlinear_case3.csv']
    ...
    subroutine set_initial_conditions(domain, forcing_case)
        ...
        ! Dispesive solver needs wall boundaries
        !domain%U(1:2,:,ELV) = 1.0_dp
        !domain%U(domain%nx(1)-1:domain%nx(1),:,ELV) = 1.0_dp
        domain%U(:,1:2,ELV) = 1.0_dp
...
program BP06
    ...
    if(md%domains(1)%use_dispersion) then
        ...
        ! Linearly reduce dispersive terms to zero as depth changes between these two values
        md%domains(1)%ds%td1 = 0.1_dp
        md%domains(1)%ds%td2 = 0.05_dp

        ! Ensure a reasonable halo for dispersive iteration
        md%domains(1)%minimum_nesting_layer_thickness = nint(20_ip*mesh_refine)
    end if
    ...
    if(nd > 1) then
        ...
        if(md%domains(2)%use_dispersion) then
            md%domains(2)%theta = 4.0_dp
            md%domains(2)%nc_grid_output%flush_every_n_output_steps = 1_ip

            ! Linearly reduce dispersive terms to zero as depth changes between these two values
            md%domains(2)%ds%td1 = 0.1_dp
            md%domains(2)%ds%td2 = 0.05_dp

            ! Ensure a reasonable halo for dispersive iteration
            md%domains(2)%minimum_nesting_layer_thickness = nint(30_ip*mesh_refine) ! Covering about 3x depth
        end if
    end if 
