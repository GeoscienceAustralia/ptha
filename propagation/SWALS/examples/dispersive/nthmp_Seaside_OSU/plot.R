...
if(round(x[[1]]$lower_left_corner[1]) == 5){
    ts5 = read.table('../../nthmp/Seaside_OSU_model/problem_data/ts_5m.txt')
}else if(round(x[[1]]$lower_left_corner[1]) == 2){
    ts5 = read.table('../../nthmp/Seaside_OSU_model/problem_data/ts_from_observations_at_wg1.txt', header=FALSE)
}
wg_all = read.table('../../nthmp/Seaside_OSU_model/problem_data/Wavegage.txt', header=TRUE)
...
image.plot(x[[1]]$xs, x[[1]]$ys, x[[1]]$elev0, zlim=c(0, 1.6), add=FALSE, 
           xlim = range(c(x[[1]]$xs), c(x[[2]]$xs)),
           ylim = range(c(x[[1]]$ys), c(x[[2]]$ys)),
...
all_gauges = read.csv('../../nthmp/Seaside_OSU_model/problem_data/point_gauge_locations.csv', header=TRUE)
...
polygon(domain_bbox$domain_interior_bbox[[1]], border='red')
polygon(domain_bbox$domain_interior_bbox[[2]], border='red')
city_gauges = read.csv('../../nthmp/Seaside_OSU_model/problem_data/Gauge_locations_near_city.csv', header=TRUE)
...
time_nudge = 0.7
data_wg = read.table('../../nthmp/Seaside_OSU_model/problem_data/Wavegage.txt', header=TRUE)
...
#
gauge_files = paste0('../../nthmp/Seaside_OSU_model/problem_data/other/Location_', gauge_label, '.txt')
