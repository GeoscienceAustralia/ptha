# NTHMP tsunami currents benchmark problem 4 (dispersive version): Seaside OSU experiment of tsunami inundation through a city
...
Figures 1 and 2 below show the [SWALS model](model.f90) structure, elevation data and locations of gauges. The model uses two nested domains with 4x grid refinement near the "city". **Only the deeper water domain uses dispersion** which is tapered to zero at depths (below MSL) of 0.2-0.1m. The other domain has elevations mostly above the SWALS `msl_linear` where we would turn off dispersion regardless, and it helps for stability to avoid it entirely on this domain. 
