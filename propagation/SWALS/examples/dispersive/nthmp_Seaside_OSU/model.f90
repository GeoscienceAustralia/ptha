...
module local_routines 
    ...
    character(len=charlen), parameter :: &
        gauge_boundary_file = '../../nthmp/Seaside_OSU_model/problem_data/ts_5m.txt'
    ...
    real(dp), parameter :: &
        global_lw(2) = [43.60_dp, 8.55_dp] - global_ll + [5.0_dp, 0.0_dp]
    ...
    subroutine set_initial_conditions(domain)
        ...
        call elevation_data%initialise(&
            [character(len=charlen) :: '../../nthmp/Seaside_OSU_model/problem_data/bathy_raster.tif'])
...
program Seaside_OSU
    ...
    integer, parameter :: nest_ratio = 4_ip ! Spatial refinement of nested grid cell size
    
    !real(dp), parameter ::  global_dt = 0.005_dp / mesh_refine ! 'leapfrog_nonlinear' needs smaller dt 
    ...
    integer(ip), parameter:: global_nx(2) = nint(global_lw/0.1_dp, ip) * mesh_refine

    ! Useful misc variables
    integer(ip):: j, i, nd, forcing_case
    character(len=20) :: tempchar
    ! Convenience variables to partition domains with/without dispersion
    real(dp), parameter :: x1 = nint(global_nx(1) * 0.55_dp) * 1.0_dp/global_nx(1)
        ! LHS domain has dispersion (fraction [0, x1] of the x-axis)
    ...
    ! Main domain, LHS fraction of x-axis ([0, x1])
    md%domains(1)%lower_left =global_ll
    md%domains(1)%lw = global_lw * [x1, 1.0_dp]
    md%domains(1)%nx = nint(global_nx * [x1, 1.0_dp])
    md%domains(1)%dx = md%domains(1)%lw/md%domains(1)%nx
    md%domains(1)%timestepping_refinement_factor = 1_ip
    md%domains(1)%dx_refinement_factor = 1.0_dp
    md%domains(1)%timestepping_method = 'midpoint' !'leapfrog_nonlinear' !default_nonlinear_timestepping_method
    md%domains(1)%use_dispersion = .true.
    if(md%domains(1)%use_dispersion) md%domains(1)%minimum_nesting_layer_thickness = 25_ip

    ! Main domain, high res, RHS fraction of x-axis ( [x1, 1.0] )
    md%domains(2)%lower_left = md%domains(1)%lower_left + [md%domains(1)%lw(1), 0.0_dp]
    md%domains(2)%lw = global_lw * [1.0_dp - x1, 1.0_dp] 
    md%domains(2)%nx = nint(global_nx * [1.0_dp - x1, 1.0_dp]) * nest_ratio
    md%domains(2)%dx = md%domains(2)%lw/md%domains(2)%nx
    md%domains(2)%timestepping_refinement_factor = nest_ratio
    md%domains(2)%dx_refinement_factor = 1.0_dp
    md%domains(2)%timestepping_method = 'midpoint' !default_nonlinear_timestepping_method
    md%domains(2)%use_dispersion = .false.
    if(md%domains(2)%use_dispersion) md%domains(2)%minimum_nesting_layer_thickness = 25_ip
    ...
    do j = 1, size(md%domains)
        ! Taper dispersion to zero between 0.2-0.1 m depth below MSL
        md%domains(j)%ds%td1 = 0.2
        md%domains(j)%ds%td2 = 0.1

        call set_initial_conditions(md%domains(j))
    end do

    call md%set_point_gauges_from_csv(&
        '../../nthmp/Seaside_OSU_model/problem_data/point_gauge_locations.csv', skip_header=1_ip)
