module local_routines 
    !!
    !! Setup a radially nonsymmetric problem with potential flow in spherical coordinates
    !! Use a latitude away from the equator to provide an opportunity to find problems
    !! in the spherical metric terms in the dispersion operator
    !!
    ...
    subroutine set_initial_conditions_nonsymmetric_radial_potential(domain)
        !
        ...
        ! Stage from a file
        call stage_data%initialise([character(len=charlen):: 'initial_condition_file_spherical.tif'])
...
program nonsymmetric_radial_potential
    !!
    !! Radially nonsymmetric problem for potential flow
    !!
    use global_mod, only: ip, dp, charlen,&
        default_linear_timestepping_method, default_nonlinear_timestepping_method,&
        radius_earth, pi
    ...
    real(dp), parameter :: central_lonlat(2) = [0.0_dp, 45.0_dp]
    ...
    ! length/width in spherical coordinates
    real(dp), parameter :: global_lw(2) = global_lw_cartesian/&
        (2.0_dp * pi * radius_earth * [cos(central_lonlat(2)/180.0_dp*pi), 1.0_dp])*360.0_dp
...
#ifndef SPHERICAL
    write(log_output_unit, *) 'ERROR: Code assumes spherical coordinates'
    call generic_stop
#endif
    ...
    !
    md%domains(1)%timestepping_method = 'midpoint' !default_linear_timestepping_method !'leapfrog_nonlinear' !'rk2' ! 
    ...
    do j = 1, size(md%domains)
        call set_initial_conditions_nonsymmetric_radial_potential(md%domains(j))
    end do
