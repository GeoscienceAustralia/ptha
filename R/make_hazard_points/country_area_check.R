...
## Why? So that we don't cut out any countries when simplifying polygons for
## GAR
## 
...
for(i in 1:ncountries){
    print(i)
    land_area_list[[i]]=unlist( 
                        lapply(wb[i,]@polygons[[1]]@Polygons, 
                            myfun<-function(x) areaPolygon(x@coords)) )
    land_area_maxind[i]=which.max(land_area_list[[i]])
    land_area_coord[i,] = wb[i,]@polygons[[1]]@Polygons[[land_area_maxind[i]]]@coords[1,]
}

land_area_sp=SpatialPointsDataFrame(land_area_coord, data=wb@data, proj4string=CRS(proj4string(wb)))
