...
code directly supports 'filtering-before-combining' (see codes toward the end
of the
[../combine_tsunami_sources/combine_tsunami_sources.R](../combine_tsunami_sources/combine_tsunami_sources.R)
script for 'filtering-after-combining'). Mathematically the problem is linear,
and there should be no difference between these two cases. However, in practice
our Kajiura filtering algorithm involves some interpolation to move to and from
