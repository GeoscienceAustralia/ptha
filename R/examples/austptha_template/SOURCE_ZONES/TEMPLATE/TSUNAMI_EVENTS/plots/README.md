...
Please look carefully at the function `plot_model_gauge_vs_data_gauge` inside [gauge_summary_statistics.R](./gauge_summary_statistics.R) to understand what everything is. In particular, the time-ranges used for various statistics are rather involved, and there are some documentation errors on this in our reports. For the stage-range statistic, and the time-series that are stored in `data_t, data_s`, the start time (seconds after the earthquake) is initially taken as MAX(0, MIN("time at which model exceeds 1/2% of its maxima", "10 minutes before observed maxima")), but is also limited to the time at which the DART buoy measurements have 1 min or faster frequency. The end-time is not more than 12 hours after the start-time (incorrectly stated as 3 hours in our report), but is likewise limited to the time-period of high-frequency DART measurements -- which more often limits the time range. The goodness-of-fit type statistic is computed over a possibly shorter time-interval -- the end-time is at most 3 hours after the observed maxima in the previously defined range (incorrectly described as "at most 3 hours" in our reports). It is reasonable to use a shorter time-period for this statistic because it is sensitive to any later-time phase-errors in the model - which are common - as opposed to the stage-range statistic, which gives a crude size measure.
