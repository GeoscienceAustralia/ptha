...
for(RdataFile in all_Rdata){
    ...
    for(stat_measure in 1:3){
        ...
        png(output_png, width=10, height = 5, units='in', res=300)
        par(mfrow=c(1,2))
        hist(stoc_score_median, freq=FALSE, main=paste0('Stochastic slip model median GoF: ', stat_name), 
            xlim=c(-0.1,1.1), col='red', density=20)
        hist(unif_score_median, freq=FALSE, main=paste0('Uniform slip model median GoF: ', stat_name), 
        ...
        legend('center', 
            c('Data', paste0('Stoch. slip (', si, ')'), paste0('Unif. slip (', ui, ')')), 
            bty='n',
            lty=c('solid', 'solid', 'longdash'),
            col=c('black', 'red', 'blue'), pch=c(19, NA, NA), cex= 2.0 + (nrow > 2),
            pt.cex=1,
            ncol=2) 
