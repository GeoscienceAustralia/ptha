...
plot_wave_heights_at_a_station<-function(lon_p, lat_p, source_zone, 
    ...
    if(slip_type == 'uniform'){
        ...
        nc_file_ind = grep(paste0('_tsunami_', source_zone, '.nc'), 
...
#
#
plot_station_deaggregated_hazard<-function(deaggregated_hazard, scale = 1, background_raster=NULL,
    main=''){
    ...
    contrib_range_individual = lapply(
        deaggregated_hazard, f<-function(x) range(x$contribution))

    # For plotting, it's useful to plot the big sources first, so they are not
    # obscured by others
    contrib_range_order = order(unlist(lapply(contrib_range_individual, max)))

    contrib_range = range(unlist(contrib_range_individual))
    ...
    for(i in contrib_range_order){
        ...
        colz = floor( sqrt((xx$contribution - 0)/
                           (contrib_range[2] - 0)) * ncol)
        colz = pmax(colz, 1)
    ...
    if(!is.null(background_raster)){
        ...
        rtemp = setValues(rtemp, seq(0,1,len=nlegend))
...
plot_source_zone_stage_vs_exceedance_rate<-function(
    ...
    site_rates_uniform = ncvar_get(rates[[site]], paste0(shear_modulus_type, 'uniform_slip_rate'))
    site_rates_stochastic = ncvar_get(rates[[site]], paste0(shear_modulus_type, 'stochastic_slip_rate'))
    site_rates_variable_uniform = ncvar_get(rates[[site]], paste0(shear_modulus_type, 'variable_uniform_slip_rate'))
