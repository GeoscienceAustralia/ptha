...
peak_stage = '/g/data/fj6/PTHA/AustPTHA_1/EVENT_RATES/revised1_tsunami_stages_at_fixed_return_periods'

library(rptha)
clip_pol = readOGR(clip_polygon, layer='clip_polygon')
peak_stg = readOGR(peak_stage,   layer='revised1_tsunami_stages_at_fixed_return_periods')

keep = which(c(!is.na(over(peak_stg, clip_pol))) & peak_stg$elev < elevation_threshold)

output = peak_stg[keep, c('lon', 'lat', 'elev', 'ST_1000')]

writeOGR(output, dsn='revised1_tsunami_stages_1_in_1000_Australia', 
    layer='revised1_tsunami_stages_1_in_1000_Australia', driver='ESRI Shapefile',
    overwrite=TRUE)

system('zip -r revised1_tsunami_stages_1_in_1000_Australia.zip revised1_tsunami_stages_1_in_1000_Australia')

# Copy the zip file to gdata
file.copy(from='revised1_tsunami_stages_1_in_1000_Australia.zip', 
    to=paste0(dirname(peak_stage), '/revised1_tsunami_stages_1_in_1000_Australia.zip'),
    overwrite=TRUE)
