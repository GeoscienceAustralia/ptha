...
#input_args = c('../../DATA/ELEV/GA250_1m/GA_250_1mx1m.tif', '-20.0',  '20.0', '60', '3', 'OUTPUTS/OUTPUT_GA250_20m', '')

###############################################################################
## INPUT DATA 

# Main raster file used for contour creation
raster_infile = input_args[1] #'../../../../DATA/ELEV/GEBCO_08/gebco_08_W-180-E180-S-90-N90-1mx1m.nc'

# Hazard contour level (m). Elevation at which we 'desire' hazard points to be
# located (e.g. -100 for 100m water depth). 
hazard_contour_level = as.numeric(input_args[2]) #-100 

# Spacing of hazard points along the contour (km)
hazard_pt_spacing = as.numeric(input_args[3]) #25 
...
if(input_args[7] == ''){
    ...
    extra_manual_haz_lines = input_args[7] #'./INPUTS/EXTRA_HAZ_LINES/EXTRA_HAZ_LINES.shp' #NULL
}
# Use a 20km spacing on the extra manual hazard point lines
extra_manual_haz_point_spacing = 20

# Name of a polygon shapefile, used as a mask. We will not include hazard
# points inside this polygon, unless they are part of the 'extra manual' line
# shapefile.
# Set to NULL if there is no mask
haz_pts_mask = './INPUTS/HAZ_PT_REMOVAL_REGION/HAZ_PT_REMOVAL_REGION.shp' #NULL 
...
# match with the tsunami model domain. Of course it is easy to change this
# with post-processing later
haz_pt_lowerleft = -180 

# All outputs will go inside this directory, in the current working directory 
output_base_dir = input_args[6] #'OUTPUTS'
...
out_tif_dir = paste0(output_base_dir, '/raster_tmp')
...
dem_0m = cu$gdal_contour(temp_tif, contour_levels=-0.0001, 
    out_dsn=paste0(output_base_dir, '/ZERO_CONTOUR'), 
    contour_shp_name='ZERO_CONTOUR')
###############################################################################
...
writeOGR(dem_0m_trim, dsn=paste0(output_base_dir, '/ZERO_CONTOUR_TRIMMED'), 
...
gdal_rasterize_command = paste(
    ...
    '-a_nodata 0.0',
    paste0(' ', output_base_dir, '/ZERO_CONTOUR_TRIMMED/ZERO_CONTOUR_TRIMMED.shp'),
    paste0(output_base_dir,'/raster_tmp/raster_tmp.tif'),
    ' -co COMPRESS=DEFLATE -co BIGTIFF=YES')
system(gdal_rasterize_command)

# Make a copy of this raster -- we will edit 2 x independently
system(paste0('cp ', output_base_dir, '/raster_tmp/raster_tmp.tif ', output_base_dir, 
    '/raster_tmp/raster_tmp3.tif'))
...
gdal_changenodata_command = paste(
    ...
    paste0(' ', output_base_dir, '/raster_tmp/raster_tmp.tif'),
    paste0(' ', output_base_dir, '/raster_tmp/raster_tmp2.tif'),
    ' -co COMPRESS=DEFLATE -co BIGTIFF=YES')
...
gdal_fillnodata_command = paste(
    ...
    paste0(output_base_dir, '/raster_tmp/raster_tmp3.tif'),
    ' -co COMPRESS=DEFALTE -co BIGTIFF=YES')
system(gdal_fillnodata_command)
...
gdal_changenodata_command = paste(
    'gdal_translate -a_nodata -1.0',
    paste0(' ', output_base_dir, '/raster_tmp/raster_tmp3.tif'),
    paste0(' ', output_base_dir, '/raster_tmp/raster_tmp4.tif'),
    ' -co COMPRESS=DEFLATE -co BIGTIFF=YES')
system(gdal_changenodata_command)
...
gdal_calc_command=paste(
    'gdal_calc.py -A ', 
    paste0(output_base_dir, '/raster_tmp/raster_tmp2.tif'),
    ' -B ', paste0(output_base_dir, '/raster_tmp/raster_tmp4.tif'),
    " --calc='(A-B)'",
    paste0(' --outfile=', output_base_dir, '/raster_tmp/raster_tmp5.tif'),
...
gdal_calc_command = paste(
    ...
    paste0(" -B ", output_base_dir, "/raster_tmp/raster_tmp5.tif"), 
    paste0(" -C ", output_base_dir, "/raster_tmp/raster_tmp4.tif"),
    " --calc=' A*(B==1) + ", 
...
dem_haz_cont = cu$gdal_contour(outfile_gdc, contour_levels=contour_depth, 
    out_dsn=paste0(output_base_dir, '/HAZARD_CONTOUR'), 
...
haz_pts = pu$cut_hazpts_in_poly(
    haz_pts, 
    haz_cull_poly_name = haz_pts_mask, 
    new_haz_pts_name = NULL, 
    lower_left=haz_pt_lowerleft, 
...
if(!is.null(extra_manual_haz_lines)){
    ...
    extra_haz_pts = rptha::approxSpatialLines(extra_haz_lines, 
        spacing=extra_manual_haz_point_spacing, longlat=TRUE)

    haz_pts = rbind(haz_pts, extra_haz_pts)
...
# Again adjust the lower left, in case we introduced more problematic
# points with the 'extra_haz_lines'. Do not 'cull' points, since we
# already did that above, and do not want to do it to the 'extra_haz_lines'
# points. But save the output points
haz_pts = pu$cut_hazpts_in_poly(
    haz_pts, 
    haz_cull_poly_name = NULL,
    new_haz_pts_name = 'HAZ_NEW', 
    lower_left= haz_pt_lowerleft,
    outdir=output_base_dir)
...
# Write out
writeOGR(haz_pts, dsn=paste0(output_base_dir, '/HAZ_PTS'), 
    layer='HAZ_PTS', driver='ESRI Shapefile', overwrite=TRUE)

pu$haz_pts_2_ursga_format(haz_pts, 
    outfile=paste0(output_base_dir, '/haz_pts_W', haz_pt_lowerleft,'.txt'))
