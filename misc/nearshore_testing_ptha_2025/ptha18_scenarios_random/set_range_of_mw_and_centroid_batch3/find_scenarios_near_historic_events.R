...
target_events = list(
    ...
    'kermadec2021-batch3' = list( 
        # Mw centred around the GCMT estimate of 8.1                                 
        Mw_lower = 8.1 - 0.15, 
        Mw_upper = 8.1 + 0.15, 
        event_point_1 = c(184.175, -26.982), # Coordinate ranges based on Romano et al. (2021)
        event_point_2 = c(182.814, -31.106), # Generous - the main slip patch is clearly in the middle
        event_time = strptime('2021-03-04 19:29:02', format='%Y-%m-%d %H:%M:%S', tz='Etc/GMT'),
        tsunami_event_dir = '/g/data/fj6/PTHA/AustPTHA_1/SOURCE_ZONES/kermadectonga2/TSUNAMI_EVENTS/'
        ),

    'newhebrides2021-batch3' = list( 
        # Mw centred around the GCMT estimate of 8.1                                 
        Mw_lower = 7.7 - 0.15, 
        Mw_upper = 7.7 + 0.15, 
        # In the absence of an inversion, 
        # Bound within +- the typical scaling relation width (~100km)
        event_point_1 = c(172.460, -22.89), # This is 2 unit-sources (+100km) along from the one with the GCMT centroid
        event_point_2 = c(170.650, -22.59), # This is 2 unit-sources (-100km) along from the one with the GCMT centroid
        event_time = strptime('2021-02-10 13:20:23', format='%Y-%m-%d %H:%M:%S', tz='Etc/GMT'),
        tsunami_event_dir = '/g/data/fj6/PTHA/AustPTHA_1/SOURCE_ZONES/newhebrides2/TSUNAMI_EVENTS/'
        ),

    'sandwich2021-batch3' = list( 
        # Mw centred around the GCMT estimate of 8.1                                 
        Mw_lower = 8.3 - 0.15, 
        Mw_upper = 8.3 + 0.15, 
        # In the absence of an inversion, 
        # Bound within +- the typical scaling relation width (~240km)
        event_point_1 = c(-24.390, -57.44), # This is 5 unit-sources (+250km) along from the one near the GCMT centroid
        event_point_2 = c(-26.600, -60.37), # This is 4 unit-sources (-200km) along from the one near the GCMT centroid
        event_time = strptime('2021-08-12 18:35:25', format='%Y-%m-%d %H:%M:%S', tz='Etc/GMT'),
        tsunami_event_dir = '/g/data/fj6/PTHA/AustPTHA_1/SOURCE_ZONES/sandwich/TSUNAMI_EVENTS/'
        )
