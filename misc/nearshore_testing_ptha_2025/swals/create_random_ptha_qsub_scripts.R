...
raster_files = Sys.glob('../ptha18_scenarios_random/set_range_of_mw_and_centroid/*/*.tif')
model_names = paste0('ptha18_random_like_historic_', gsub('.tif', '', basename(raster_files), fixed=TRUE))

jobs_per_qsub_file = 12 # Run this many models in a single qsub file, to stay within NCI walltime limits
number_of_qsub_files = length(raster_files)/jobs_per_qsub_file
if(round(number_of_qsub_files) != number_of_qsub_files){
    stop('jobs_per_qsub_file does not evenly divide number of raster files -- I assume it does')
}

# Mapping between the jobs and the qsub file. This cyclic distribution should make the work fairly even.
job_to_qsub_file_index = rep(1:number_of_qsub_files, times=jobs_per_qsub_file)
...
for(i in 1:number_of_qsub_files){
    output_qsub_filename = paste0('run_ptha18_random_like_historic_batch_', 100 + i, '.sh')
