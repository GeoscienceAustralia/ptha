...
raster_files = c(
    Sys.glob('../ptha18_scenarios_random/set_range_of_mw_and_centr*/*/*umatra2004*.tif'),
    Sys.glob('../ptha18_scenarios_random/set_range_of_mw_and_centr*/*/*umatra2005*.tif'))
model_names = paste0('ptha18_random_like_historic_', gsub('.tif', '', basename(raster_files), fixed=TRUE))

jobs_per_qsub_file = 7
...
with(model_run_creation_env, {
    ...
    make_job_command<-function(raster_file, model_name){
        ...
        if(grepl('sumatra2005', model_name)){
            # For the Sumatra 2005 simulations, only include South West WA
            highres_region = 'WA'
        }else{
            highres_region = 'australiaWA'
        }
        if(! any(highres_region == c('WA', 'australiaWA'))) stop('unknown highres region')


        # Choose load balance file based on highres_region and offshore model type
        if(highres_region == 'WA'){
            load_balance_file = 'load_balance_files/load_balance_wa_8nodes_32MPI.txt'
        }else if(highres_region == 'australiaWA'){
            load_balance_file = 'load_balance_files/load_balance_australiaWA_8nodes_32mpi.txt'
        }else{
        ...
        job_command = paste0(
            ...
            '-x OMP_NUM_THREADS=', omp_num_threads, ' -x OMP_PROC_BIND=true ', 
...
for(i in 1:number_of_qsub_files){
    output_qsub_filename = paste0('run_ptha18_extendWA_random_like_historic_batch_', 100 + i, '.sh')
    ...
    for(j in 1:length(jobs_to_run)){
        ...
        # Tar the folder
        job_run_commands[j] = paste0(job_run_commands[j], 'Rscript tar_and_remove_matching_dir.R OUTPUTS_2025_extend_WA/', model_names[ind], '\n')
        # Make a newline
        job_run_commands[j] = paste0(job_run_commands[j], '\n')
    }
