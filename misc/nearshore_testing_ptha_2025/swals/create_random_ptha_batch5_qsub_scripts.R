...
raster_files = Sys.glob('../ptha18_scenarios_random/set_range_of_mw_and_centroid_batch5/*/*.tif')
...
with(model_run_creation_env, {
    ...
    make_job_command<-function(raster_file, model_name){
        ...
        if(! any(highres_region == 'perth')) stop('unknown highres region')


        # Choose load balance file based on highres_region and offshore model type
        if(highres_region == 'perth'){
...
for(i in 1:number_of_qsub_files){
    output_qsub_filename = paste0('run_ptha18_batch5_random_like_historic_batch_', 100 + i, '.sh')
