...
raster_files = Sys.glob('../ptha18_scenarios_random/set_range_of_mw_and_centroid_batch2/*/*.tif')
...
## Something changed on the NCI environment, and now we need
## to use newer variants of software or else SWALS hangs with 
## multi-node output [even though it worked previously].
...
for(i in 1:number_of_qsub_files){
    output_qsub_filename = paste0('run_ptha18_batch2_random_like_historic_batch_', 100 + i, '.sh')
