...
program run_model
    ...
    !!         'NSW' [only use NSW high-res domains] or 'Perth' [only use WA highres domains]
    !!
    ...
    if(highres_regions == 'australia' .or. highres_regions == 'perth') then
        !! Only include highres Vic if we are doing an 'australia-wide'
        ...
        ! Index of the regional perth domain
        if(highres_regions == 'australia') then
            perth_regional = nd_global + nd_nsw + nd_victoria + 1
        else if(highres_regions == 'perth') then
            perth_regional = nd_global + 1
        end if
