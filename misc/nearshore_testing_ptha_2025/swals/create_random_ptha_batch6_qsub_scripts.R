...
raster_files = Sys.glob('../ptha18_scenarios_random/set_range_of_mw_and_centroid_batch6/*/*.tif')
...
with(model_run_creation_env, {
    ...
    make_job_command<-function(raster_file, model_name){
        ...
        if(grepl('sumatra2007', model_name) | grepl('java2006', model_name)){
            # These events only have data in NWWA
            highres_region = 'NWWA'
        }
        if(! any(highres_region == 'NWWA')) stop('unknown highres region')


        # Choose load balance file based on highres_region and offshore model type
        if(highres_region == 'NWWA'){
            load_balance_file = 'load_balance_files/load_balance_nwwa_8nodes_32mpi.txt'
        }else{
...
for(i in 1:number_of_qsub_files){
    output_qsub_filename = paste0('run_ptha18_batch6_random_like_historic_batch_', 100 + i, '.sh')
    ...
    for(j in 1:length(jobs_to_run)){
        ...
        # Tar the folder
        job_run_commands[j] = paste0(job_run_commands[j], 'Rscript tar_and_remove_matching_dir.R OUTPUTS_2025_NWWA/', model_names[ind], '\n')
        # Make a newline
        job_run_commands[j] = paste0(job_run_commands[j], '\n')
    }
