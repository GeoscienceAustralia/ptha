...
plot_and_print_statistics<-function(sz, outdir=OUTDIR){
    ...
    for(plot_site in c('target_pt','tweed_offshore')){
        ...
        if(ci_method == 'normal'){
            ...
            # Attempt at using a beta distribution to characterise the uncertainties
            # As with the normal distribution, this is only an approximation, and my quick tests didn't suggest it was much better.
            ...
            estBetaParams <- function(mu, var) {
              ...
              # Should check that 0 < mu < 1 and 0 < var < mu*(1-mu)
              alpha <- ((1 - mu) / var - 1 / mu) * mu ^ 2
              beta <- alpha * (1 / mu - 1)
              return(cbind(alpha, beta))
              #return(params = list(alpha = alpha, beta = beta))
            }
            # Assume the Monte Carlo solution varies like a beta distribution rescaled to [0, TOTAL_RATE]
            # A regular beta distribution ranges from [0,1], so we have to just the mean and variance accordingly.
            TOTAL_RATE = sum(event_rate_logic_tree_mean)
            beta_pars = estBetaParams(mu=analytical_exrate/TOTAL_RATE, var=mc_exrate_variance/TOTAL_RATE**2)
            lower_ci = qbeta(0.025, shape1=beta_pars[,1], shape2=beta_pars[,2])*TOTAL_RATE
            upper_ci = qbeta(0.975, shape1=beta_pars[,1], shape2=beta_pars[,2])*TOTAL_RATE
            points(stage_thresholds, lower_ci, t='l', col='black', lwd=2, lty='dashed')
            points(stage_thresholds, upper_ci, t='l', col='black', lwd=2, lty='dashed')
        }
        #legend('topright', 
        ...
        legend('topright', '1000 Monte Carlo samples\n(reference site ID710.5)', lty='solid', lwd=1.5, col='grey', cex=1.6, bg='white', box.col='white')
        mtext('Exceedance rate (events/year)', side=2, cex=1.6, line=6)
        dev.off()
        ...
        for(i in 1:Ntest){
            ...
            if(ci_method == 'normal'){
                ...
                # Try beta instead of normal. Doesn't seem to be much better, added complexity, not worth it?
                beta_pars = estBetaParams(mu=test_var$exrate[1]/TOTAL_RATE, var=test_var$exrate_var[1]/TOTAL_RATE**2)
                ci_lower[i] = qbeta(0.025, shape1=beta_pars[,1], shape2=beta_pars[,2])*TOTAL_RATE
                ci_upper[i] = qbeta(0.975, shape1=beta_pars[,1], shape2=beta_pars[,2])*TOTAL_RATE
            }
        }
