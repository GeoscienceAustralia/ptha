#
# Code to compute the unit sources used in 
...
# NOTE: Here we deliberately do not compute the summed source. Our tsunami model will
# use the unit sources, combined with slip reported in the summary txt file, to impose a time-varying source.
...
#
#
x = read.table('Fuji_satake_2004_joint_inversion_1ms.txt', header=TRUE, colClasses=rep('numeric', 7))
lat = x$lat # rupture location in DEGRESS N, WGS84
lon = x$lon # rupture location in DEGREES E, WGS84 

width          =   rep(100.0, length(lon)) # km -- rupture width
length         =   rep(100.0, length(lon)) # km -- rupture width
dip            =   x$dip # Degrees -- rupture dip
rake           =  x$rake  # Degrees -- rupture rake
depth = x$depth # km -- depth of rupture (top)
strike         =    x$strike # Degrees 
## Slip from JOINT inversion
slip = x$slip # NOTE: Below we deliberately compute the unit sources (assume 1m slip). But we will use the slip values written to the log file in the actual source creation.
rupture_name   =    as.character(1:length(lon)) # A name, which will appear in the filename
rupture_basename = 'Fuji_andaman2004_unit_sources' # Name at the start of output files
make_plot = FALSE
