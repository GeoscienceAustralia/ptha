# Example from 
#
# (https://doi.org/10.1029/2022EA002346) Gusman, A. R.; Roger, J.; Power, W.; Fry, B. & Kaneko, Y. The 2021 Loyalty Islands Earthquake (Mw 7.7): Tsunami Waveform Inversion and Implications for Tsunami Forecasting for New Zealand Earth and Space Science, 2022, 9, e2022EA002346  
#
#
...
#
#
x = read.table('source_inversion_GusmanEtAl.txt', header=TRUE, colClasses='numeric')
lon = x$lonc
lat = x$latc
width  = x$width # rupture width in km.
length = x$length # rupture length in km
strike = x$strike # Strike in degrees clockwise from N
dip = x$dip # Dip
depth = x$depthc # From sin(dip/180*pi)*width, it's clearly that depths = centroid with 1km top-depth at trench
rake  = x$rake # Degrees -- rupture rake
slip = x$slip # m
rupture_name   =   as.character(1:length(lon)) # A name, which will appear in the filename
rupture_basename = 'Gusman_NewHebrides_sources' # Name at the start of output files
make_plot = FALSE

if(make_plot){
    # Quick check that the slip matches the picture in the paper
    plot(lon, lat, 
         col=colorRampPalette(c('white', 'yellow', 'orange', 'red', 'black'))(ceiling(max(slip*10)+1))[floor(slip*10)+1], 
         asp=1/cos(lat[1]/180*pi), pch=19, cex=2)
...
#
output_res = 1/60 # degrees
output_lons = seq(floor(min(lon))-5, ceiling(max(lon))+5, by=output_res)
output_lats = seq(floor(min(lat))-5, ceiling(max(lat))+5, by=output_res)
