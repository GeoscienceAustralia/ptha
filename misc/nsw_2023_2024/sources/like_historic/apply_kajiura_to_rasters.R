...
recreate_existing_files = TRUE # If TRUE, remake all files. If FALSE, don't overwrite files that already exist. 

## Elevation DEM (effects kajiura smoothing kernel scale)
elevation_rast_home = '/media/gareth/Windows7_OS/Users/gareth/Documents/work/AustPTHA/MODELS/AustPTHA_c/DATA/ELEV/merged_dem/merged_gebco_ga250_dem_patched.tif'
elevation_rast_NCI = '/g/data/w85/tsunami/MODELS/inundation/australia_wide/clean_version/elevation/derived_for_model/global/ptha18/merged_gebco_ga250_dem_patched.tif'
elevation_rast = ifelse(file.exists(elevation_rast_NCI), elevation_rast_NCI, elevation_rast_home)

## Raster files containing Okada deformation, which should have kajiura smoothing applied.
raster_files = commandArgs(trailingOnly=TRUE)
#raster_files = c(
