...
# with nonlinear model (high-res, 40 hours) using importance sampling.
#
# MULTIPLE IMPORATANCE SAMPLING VERSION
#
...
file_home = '../../../../../../../AustPTHA/CODE/ptha/propagation/SWALS/plot.R'
source(ifelse(file.exists(file_nci), file_nci, file_home), local=swals)
...
# Names of each of the importance samples that is combined herein using
# multiple importance sampling.
all_IS_samples = c('scenarios_ID710.5', 'scenarios_ID1315.5', 'scenarios_ID4186.3')

# Storage for each importance sample: 
#  raster files 
tarred_raster_files_list = vector(mode='list', length=length(all_IS_samples))
names(tarred_raster_files_list) = all_IS_samples
#  scenarios
importance_sampling_scenarios_logic_tree_mean_list = tarred_raster_files_list
#  sample weights
weights_importance_sampling_list = tarred_raster_files_list

# Get raster_output_files.tar for each importance sample
tarred_raster_files_list[['scenarios_ID710.5']] = 
    Sys.glob('../../swals/OUTPUTS/ptha18-NSW2023b-ID710.5-sealevel110cm/*/*/raster_output_files.tar')
tarred_raster_files_list[['scenarios_ID1315.5']] = 
    Sys.glob('../../swals/OUTPUTS/ptha18-NSW2023-ID1315.5-sealevel110cm/*/*/raster_output_files.tar')
tarred_raster_files_list[['scenarios_ID4186.3']] = 
    Sys.glob('../../swals/OUTPUTS/ptha18-NSW2023-ID4186.3-sealevel110cm/*/*/raster_output_files.tar')
stopifnot(all(names(tarred_raster_files_list) %in% all_IS_samples))

tarred_raster_files = do.call(c, tarred_raster_files_list)

# Get scenarios for each importance sample
for(IS_sample in all_IS_samples){

    dirIS = paste0('../../sources/hazard/', IS_sample)

    # Files with scenarios used for importance-sampling -- logic_tree_mean curve results
    importance_sampling_scenarios_logic_tree_mean_list[[IS_sample]] = list(
        alaskaaleutians = read.csv(paste0(dirIS, 
                '/random_alaskaaleutians/random_scenarios_alaskaaleutians_logic_tree_mean_HS.csv')),
        kermadectonga2 = read.csv(paste0(dirIS, 
                '/random_kermadectonga2/random_scenarios_kermadectonga2_logic_tree_mean_HS.csv')),
        newhebrides2 = read.csv(paste0(dirIS,
                '/random_newhebrides2/random_scenarios_newhebrides2_logic_tree_mean_HS.csv')),
        outerrise_kermadectonga = read.csv(paste0(dirIS,
                '/random_outerrise_kermadectonga/random_scenarios_outerrise_kermadectonga_logic_tree_mean_HS.csv')),
        outerrisenewhebrides = read.csv(paste0(dirIS,
                '/random_outerrisenewhebrides/random_scenarios_outerrisenewhebrides_logic_tree_mean_HS.csv')),
        outerrise_puysegur = read.csv(paste0(dirIS, 
                '/random_outerrise_puysegur/random_scenarios_outerrise_puysegur_logic_tree_mean_HS.csv')),
        puysegur2 = read.csv(paste0(dirIS,
                '/random_puysegur2/random_scenarios_puysegur2_logic_tree_mean_HS.csv')),
        solomon2 = read.csv(paste0(dirIS,
                '/random_solomon2/random_scenarios_solomon2_logic_tree_mean_HS.csv')),
        southamerica = read.csv(paste0(dirIS, 
                '/random_southamerica/random_scenarios_southamerica_logic_tree_mean_HS.csv'))
            )

    # For each source zone, get the weight for this importance sample at
    # the target point
    weights_importance_sampling_list[[IS_sample]] = lapply(
        names(importance_sampling_scenarios_logic_tree_mean_list[[IS_sample]]),
        function(source_zone) {
            wt = as.numeric(extract( rast(paste0(
                '../../sources/hazard/multiple_importance_sampling_weights/',
                'importance_sample_weight_rasters/weight_raster_',
                source_zone, '_r_1in10000_', IS_sample, '.tif')),
                matrix(target_point, ncol=2,nrow=1),
                method='bilinear'))
            # Default weight in NA regions
            if(is.na(wt)) wt = 1/length(all_samples)
            return(wt)
        }
    )
    names(weights_importance_sampling_list[[IS_sample]]) =
        names(importance_sampling_scenarios_logic_tree_mean_list[[IS_sample]])
}

# Make pooled scenarios, compute correction for variance, ...
# For each soure zone
#   for each sample j
#       rate --> rate*wt_j
#       sampling_prob --> sampling_prob * N_j/sum_j(N_j)
#   variance_correction_factor = sum_j{ N_j (wt_j/N_j - 1/sum_j{N_j})^2 }
#       This is multiplied by -1*rate_MIS**2 to get the variance adjustment
#   Pool the scenarios with these adjustments
source_zones = names(importance_sampling_scenarios_logic_tree_mean_list[[1]])
# Storage for pooled samples
importance_sampling_scenarios_logic_tree_mean = vector(mode='list', length=length(source_zones))
names(importance_sampling_scenarios_logic_tree_mean) = source_zones
# Storage for variance correction factor
source_zone_variance_correction_factor = importance_sampling_scenarios_logic_tree_mean
for(i in 1:length(source_zones)){

    source_zone = source_zones[i]
    Nj = rep(NA, length(all_IS_samples))
    wj = Nj

    # Compute the sample sizes and weights for all source zones
    for(j in 1:length(all_IS_samples)){
        IS_sample = all_IS_samples[j]
        Nj[j] = nrow(importance_sampling_scenarios_logic_tree_mean_list[[IS_sample]][[source_zone]])
        wj[j] = weights_importance_sampling_list[[IS_sample]][[source_zone]] 
    }
    stopifnot(abs(sum(wj) - 1) < 1e-06) # Weights must sum to 1

    # Compute the "pooled" rate and sampling probability
    #   for each sample j
    #       rate --> rate*wt_j
    #       sampling_prob --> sampling_prob * N_j/sum_j(N_j)
    # Also store the MIS weight, as we need it later for epistemic uncertainty calculations that compute
    # rates using PTHA18 (and thus bypass the re-weighting of scenario rates here)
    for(j in 1:length(all_IS_samples)){
        IS_sample = all_IS_samples[j]

        # Pooled rate --> rate*wt_j
        importance_sampling_scenarios_logic_tree_mean_list[[IS_sample]][[source_zone]]$event_rate_logic_tree_mean = 
            wj[j] * importance_sampling_scenarios_logic_tree_mean_list[[IS_sample]][[source_zone]]$event_rate_logic_tree_mean
        # Pooled sampling_prob --> sampling_prob * N_j/sum_j(N_j)
        importance_sampling_scenarios_logic_tree_mean_list[[IS_sample]][[source_zone]]$sampling_prob = 
            Nj[j]/sum(Nj) * importance_sampling_scenarios_logic_tree_mean_list[[IS_sample]][[source_zone]]$sampling_prob

        # Store the multiple importance sampling weight weight. Needed later if we do calculations using PTHA18 rates.
        importance_sampling_scenarios_logic_tree_mean_list[[IS_sample]][[source_zone]]$MIS_weight = rep(wj[j], Nj[j])
    }

    # Pool the scenarios
    importance_sampling_scenarios_logic_tree_mean[[source_zone]] = do.call(rbind, 
        lapply(importance_sampling_scenarios_logic_tree_mean_list, function(y) y[[source_zone]]))

    # The variance of interest is equal to the pooled variance MINUS
    # (rate_MIS)**2*source_zone_variance_correction_factor
    source_zone_variance_correction_factor[[source_zone]] = sum(Nj * (wj/Nj - 1/sum(Nj))**2)
        # For justification see my notes in
        # /media/gareth/Windows7_OS/Users/gareth/Documents/work/Inundation_tsunami/NSW/doc/reports_by_us/importance_sampling_approach/
...
for(nm in names(importance_sampling_scenarios_logic_tree_mean)){
    ...
    # to the scenario_max_stages (which only include each scenario once in a single sample, but
    # might include repeated scenarios now that we do multiple importance sampling)
    ...
    # MULTIPLE IMPORTANCE SAMPLING ADJUSTMENT to the pooled variance
    exrate_variance_adjustment_factor = exrates_with_uncertainty[,1]**2 * 
        source_zone_variance_correction_factor[[nm]]
    ...
    output_df = data.frame(max_stage = output_max_stages_IS,
        ...
        exrate_variance = exrates_with_uncertainty[,2] - exrate_variance_adjustment_factor)

    k = which(output_df$exrate_variance < 0)
    if(length(k) > 0){
        print(paste0('A. Removing negative variances on ', nm))
        output_df$exrate_variance[k] = 0
    }
    ...
    # MULTIPLE IMPORTANCE SAMPLING ADJUSTMENT to the pooled variance
    exrate_variance_adjustment_factor = exrates_with_uncertainty_using_PTHA18_max_stage[,1]**2 * 
        source_zone_variance_correction_factor[[nm]]
    ...
    output_df = cbind(output_df, data.frame(
        ...
        exrate_variance_using_ptha18_stages_plus_nonlinear_model_MSL=exrates_with_uncertainty_using_PTHA18_max_stage[,2] - exrate_variance_adjustment_factor))

    k = which(output_df$exrate_variance_using_ptha18_stages_plus_nonlinear_model_MSL < 0)
    if(length(k) > 0){
        print(paste0('B. Removing negative variances on ', nm))
        output_df$exrate_variance_using_ptha18_stages_plus_nonlinear_model_MSL[k] = 0
    }
    ...
    for(max_stage_type in c('nonlinear_model', 'ptha18')){
        ...
        for(source_zone in names(importance_sampling_scenarios_logic_tree_mean)){
            ...
            mtch = match(importance_sampling_scenarios_logic_tree_mean[[source_zone]]$inds,
                ...
                # Note that mtch might contain repeated values because
                # scenario_max_stages_SZ will only contain each scenario once
                # (in a single sample -- now we do multiple importance sampling
                # it can have repeated samples -- although they will ultimately be the same scenario).
                # Whereas importance_sampling_scenarios_logic_tree_mean might
                # contain scenarios that have been sampled more than once.
            ...
            unsegmented_stage_vs_exrate_curves = estimate_stage_exrate_curves_with_MIS_for_all_logic_tree_branches(
                ptha18_detailed,
                source_zone,
                source_zone_segment="",
            ...
            if(length(segment_names) > 0){
                ...
                for(nm_i in segment_names){
                    segmented_stage_vs_exrate_curves[[nm_i]] =
                        estimate_stage_exrate_curves_with_MIS_for_all_logic_tree_branches(
                            ptha18_detailed,
                            source_zone,
