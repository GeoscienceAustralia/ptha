...
#
estimate_stage_exrate_curves_with_MIS_for_all_logic_tree_branches<-function(
    ptha18_detailed,
    source_zone,
    source_zone_segment,
    ...
    # When we do multiple importance sampling, this may have repeated values. In that case it has
    # already been rescaled and pooled (so all samples are included).
    sampling_prob_sampled_scenarios = importance_sampling_scenarios_logic_tree_mean_on_source_zone$sampling_prob
    # Weights used in multiple importance sampling. Needed here since below, the routine computes scenario
    # rates from PTHA18 directly, and if we do multiple importance sampling they need to be rescaled to reflect the
    # weight of each sample.
    MIS_weights_sampled_scenarios = importance_sampling_scenarios_logic_tree_mean_on_source_zone$MIS_weight
    ...
    for(i in 1:NLTB){
        ...
        for(j in 1:length(threshold_stage_values)){
            exrate_by_stage_threshold[j,i] = sum(
                MIS_weights_sampled_scenarios * sampled_scenario_rate * 
                (sampled_scenarios_max_stage > threshold_stage_values[j])/
