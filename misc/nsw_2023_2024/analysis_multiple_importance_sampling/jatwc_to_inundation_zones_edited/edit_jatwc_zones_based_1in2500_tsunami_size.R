...
marine_warning_zone_shp = Sys.glob('../NSW_tsunami_modelling_project_final_outputs/JATWC_inundation_zones/Inundation_zones/*/*_marine_warning_with-PTHA-exrate-limit_84pc_4e-04/*_marine_warning_with-PTHA-exrate-limit_84pc_4e-04.shp')
marine_warning_zone_names = basename(dirname(dirname(marine_warning_zone_shp)))


clip_warning_zone<-function(zone_name, zone_file, THRESHOLD_MAX_STAGE, max_stage_1in2500_84pc){

    #zone_name = land_warning_zone_names[i]
    zone_vect = vect(zone_file) #vect(land_warning_zone_shp[i])
    zone_warning_folder_name = basename(dirname(zone_file)) #basename(dirname(land_warning_zone_shp[i]))
    ...
    output_dir = paste0(zone_name, '/', zone_warning_folder_name, '_where_1in2500_84pc_max_stage_exceeds_', THRESHOLD_MAX_STAGE)
...
for(i in 1:length(land_warning_zone_names)){
    clip_warning_zone(land_warning_zone_names[i], land_warning_zone_shp[i], THRESHOLD_MAX_STAGE, max_stage_1in2500_84pc)
}

for(i in 1:length(marine_warning_zone_names)){
    clip_warning_zone(marine_warning_zone_names[i], marine_warning_zone_shp[i], THRESHOLD_MAX_STAGE, max_stage_1in2500_84pc)
}
