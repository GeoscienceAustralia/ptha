...
#'
#' @param all_samples data.frame with pooled scenarios from multiple importance samples
...
make_all_pixel_data<-function(all_samples, raster_tar_files, 
    ...
    scenarios_to_results_inds = all_samples$matching_raster_tar_file_index
    names(scenarios_to_results_inds) = NULL #""
    stopifnot(!any(is.na(scenarios_to_results_inds)))

    # Get the weight rasters
    unique_weight_raster_files = unique(all_samples$weight_raster)
    weight_raster_file_ind = match(all_samples$weight_raster, unique_weight_raster_files)
    weight_rasters = lapply(unique_weight_raster_files, raster)
    names(weight_rasters) = unique_weight_raster_files
    ...
    # Get the weights, bilinearly interpolated onto the template raster
    default_weight = 1/length(weight_rasters)
    weight_rasters_interpolated = lapply(weight_rasters,
        function(wr){
            tmp = resample(wr, template_raster, method='bilinear')
            tmp[is.na(tmp)] = default_weight
            return(as.matrix(tmp))
        }
    )
    # Check sum of interpolated weights == 1
    wr_sum = weight_rasters_interpolated[[1]]
    if(length(weight_rasters_interpolated) > 1){
        for(i in 2:length(weight_rasters_interpolated)) wr_sum = wr_sum + weight_rasters_interpolated[[i]]
    }
    if(!all(abs(wr_sum - 1) < 1e-06)){
    ...
    for(j in 1:dim(big_depth_array)[3]){
        for(i in 1:dim(big_depth_array)[2]){
            ...
            # Mutliple importance sampling: store weights of each importance sample
            sample_weights = unlist(lapply(weight_rasters_interpolated, function(x) x[i,j]))
            stopifnot(abs(sum(sample_weights) - 1) < 1e-06)
...
#' @param importance_sample_weights vector with length matching the number of importance samples,
#' which we can use to re-weight the scenario rates for multiple importance sampling.
...
#' 
get_exrate_at_depth_and_percentile_MIS<-function(
    chosen_depth, 
    ...
    # MULTIPLE IMPORTANCE SAMPLING info
    importance_sample_weights_sampled_scenarios = importance_sample_weights[all_samples[[1]]$sample_group_index]
    importance_sample_Nj_on_sum_Nj_sampled_scenarios = all_samples[[1]]$Nj_on_sumNj
    ...
    for(nm_i in names(all_source_rate_info)){
        ...
        for(i in 1:length(chosen_depth)){
            ...
            # Adjusted for multiple importance sampling
            terms_indep_ltb = 
                importance_sample_weights_sampled_scenarios * 
                ptha18_conditional_prob_given_mw_sampled_scenarios * 
                (sampled_scenarios_max_depth > chosen_depth[i])/
                (sampling_prob_sampled_scenarios*N_scenarios * importance_sample_Nj_on_sum_Nj_sampled_scenarios)
...
get_exrate_percentile_at_pixel<-function(pixel_data, all_samples, all_source_rate_info, 
    ...
    if(!is.null(ALWAYS_WET_EXRATE[1])){
        stopifnot(length(ALWAYS_WET_EXRATE) == length(pixel_data$sample_weights))
        ...
        if(all(pixel_data$model_runs_max_value > EXCEEDANCE_THRESHOLD)) return(sum(ALWAYS_WET_EXRATE*pixel_data$sample_weights))
        ...
        if(all(ALWAYS_WET_EXRATE == 0.0)) return(0.0)
    }
    ...
    importance_sample_weights = pixel_data$sample_weights

    exrate_at_percentile = get_exrate_at_depth_and_percentile_MIS(
