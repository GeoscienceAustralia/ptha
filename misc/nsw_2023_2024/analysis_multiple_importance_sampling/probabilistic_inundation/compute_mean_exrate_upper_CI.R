...
# Any domains for which we didn't make rasters (often domain 1 is skipped for speed)
skipped_domains = 1 
...
setup_files<-function(sz_dir, threshold_of_interest){
    ...
    # How many domains?
    ND = length(exrate_rasters) + length(skipped_domains) #
    # Tag in filename that will identify the domain
    unique_domain_flags = paste0('_domain_', setdiff(1:ND, skipped_domains), '_')
