...
GLOBAL_DOMAIN_LOWER_LEFT = c(-40, -79.0)
...
# to date when I've tried it it was more efficient without.
...
#outdir_startname = 'domains_2023_04_08_' # This should have been 2023_08_08 !!!
#outdir_startname = 'domains_2023_08_23_'
outdir_startname = 'domains_2023_11_09_with_LHI_Middleton_Norfolk'
...
#
NUMBER_NESTING_LEVELS = 3 # Not counting global domain
...
#nesting[[1]]$region = readOGR('shelf_zone/shelf_zone.shp', layer='shelf_zone')
#nesting[[1]]$region = readOGR('shelf_zone_with_LHI/shelf_zone.shp', layer='shelf_zone')
#nesting[[1]]$region = readOGR('shelf_zone_with_LHI_and_NZ/shelf_zone_with_LHI_and_NZ.shp', layer='shelf_zone_with_LHI_and_NZ')
nesting[[1]]$region = readOGR('shelf_zone_with_LHI_Middleton_Norfolk/shelf_zone.shp', layer='shelf_zone')
#nesting[[1]]$domain_size = c(0.5, 0.5) # Tile size (extent of one domain prior to halo buffering)
nesting[[1]]$domain_size = c(0.5, 1.5) # Tile size (extent of one domain prior to halo buffering)
names(nesting)[1] = 'first_level_nesting'

# Two levels finer than global domain (typically inside first level)
#nesting[[2]]$region = readOGR('nearshore_zone_with_LHI', layer='nearshore_zone')
nesting[[2]]$region = readOGR('nearshore_zone_with_LHI_Middledon_Norfolk/nearshore_zone.shp', layer='nearshore_zone')
nesting[[2]]$domain_size = nesting[[1]]$domain_size * c(1/3, 1/9) # Must be an integer divisor of the coarser domain size. Also,
...
nesting[[3]]$region = readOGR('highres_sites_LHI/highres_sites_LHI.shp', layer='highres_sites_LHI')
...
for(i in seq(length(nesting), 1, by=-1)){
    ...
    region_boundaries = unique(region_boundaries) # Remove duplicates
