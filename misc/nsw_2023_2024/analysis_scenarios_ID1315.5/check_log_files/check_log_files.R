...
for(i in 1:N){
    ...
    plot_panel<-function(i, MSL=MSL_LINEAR_DEFAULT){
        ...
        if(any(!is.finite(t8$normalised_energy_on_rho))){
            ...
            abline(v=t8$before_bc_time, t='l', col='blue', lty='dashed')
            #points(t0$time, t0$normalised_energy_on_rho, t='l', col='blue')
            points(t8$time, t8$energy_on_rho, t='l', col='red', lty='dashed')
            legend('topright', c('Energy', 'Energy normalised for volume change', 'Time when |boundary fluxes| > threshold'), 
                col=c('red', 'black', 'blue'), lty=c('dashed', 'solid', 'dashed'), bty='n')
