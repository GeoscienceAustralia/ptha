...
real(dp) ::  global_ll(2) = [-40.0_dp, -79.0_dp]

! Length/width of multidomain in degrees lon,lat
real(dp) :: global_lw(2) = [360.0_dp, 68.0_dp + 79.0_dp ]
...
logical ::  use_periodic_EW_multidomain = .true.
...
real(dp) ::  global_dx_arcmin = 1.5_dp !2.0_dp
...
character(len=charlen) :: breakwalls_file_list = "../breakwalls/breakwalls_file_list.txt"
...
character(len=charlen) :: inverts_file_list = "../inverts/inverts_file_list.txt"
 
! Point gauges (where we store time-series outputs)
character(len=charlen) :: swals_point_gauge_file = &
    '../point_gauges/point_gauges_2023_11_14.csv'
    !'../point_gauges/point_gauges_2023_08_08.csv'
...
! keep file-size down. 
integer(ip) :: write_grids_every_nth_writeout_timestep = 240_ip
...
subroutine set_seconds_before_evolve(stage_file, run_type, seconds_before_evolve)
    ...
    if(index(run_type, 'test') /= 0) then
        ...
        seconds_before_evolve = 0.0_dp
    else if(index(stage_file, 'Chile') /= 0) then
        ! Chile source inversion
    ...
    else if(index(stage_file, 'FujiSatake2007_time_varying_forcing') /= 0) then
        ! Sumatra 2004 source inversion, based on Fujii and Satake 2007. 
        seconds_before_evolve = 3600 * 8.0_dp
    else if(index(stage_file, 'random_southamerica') /= 0) then
        ! South American ptha18 source
        seconds_before_evolve = 3600 * 9.0_dp
    else if(index(stage_file, 'random_alaskaaleutians') /= 0) then
        ! Alaska-aleutians ptha18 source
        seconds_before_evolve = 3600 * 7.0_dp
    end if
end subroutine
