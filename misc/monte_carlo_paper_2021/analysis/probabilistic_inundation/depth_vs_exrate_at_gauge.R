...
}else if(run_case == 'point_984.6'){
    # Case 3 -- onshore point, in an area where the difference between our 2 MC samples was more obvious. 
    TARGET_GAUGE = matrix(c(184.9063411,-21.1400389), nrow=1)
    output_rds_name = paste0(run_series_name, '_depth_and_stage_exrate_curve_at_point_984.6.RDS')
}else{
...
scenario_base = ifelse(grepl('_repeated', run_series_name),
    ...
    '../../sources/random/')
scenarios_data_files = paste0(scenario_base, c(
    'random_scenarios_kermadectonga2_hukurangi_segment_HS.csv',
...
get_max_depth_at_gauge<-function(md_dir){
    ...
    output = data.frame(lon=lon[nearest_point], lat=lat[nearest_point], elev=elev[nearest_point], 
               ...
               md_dir=md_dir, domain_index=local_domain_index)
...
for(i in 1:length(scenarios_databases)){
    scenarios_databases[[i]]$md_dir = get_md_dirs_matching_scenario_rows(
        scenarios_databases[[i]]$inds, md_dirs)
}
...
for(sd_names in names_scenarios_databases){
    ...
    for(i in 1:length(matching_inds)){
        scenario_rates[matching_inds[i]] = scenario_rates[matching_inds[i]] + 
            local_scenarios$importance_sampling_scenario_rates_basic[i]
    }
...
outputs = list(stage_exrate_curves=stage_exrate_curves, results_df = results_df, scenarios_databases=scenarios_databases)
