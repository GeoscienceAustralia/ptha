...
kt_full      = ptha18_source_rate_env$get_PTHA18_scenario_conditional_probability_and_rates_on_segment(source_zone, '')
kt_tonga     = ptha18_source_rate_env$get_PTHA18_scenario_conditional_probability_and_rates_on_segment(source_zone, 'tonga')
kt_kermadec  = ptha18_source_rate_env$get_PTHA18_scenario_conditional_probability_and_rates_on_segment(source_zone, 'kermadec')
kt_hikurangi = ptha18_source_rate_env$get_PTHA18_scenario_conditional_probability_and_rates_on_segment(source_zone, 'hikurangi')
...
write.csv(random_scenario_info$unsegmented, 'random_scenarios_kermadectonga2_unsegmented_HS.csv', row.names=FALSE)
write.csv(random_scenario_info$tonga, 'random_scenarios_kermadectonga2_tonga_segment_HS.csv', row.names=FALSE)
write.csv(random_scenario_info$kermadec, 'random_scenarios_kermadectonga2_kermadec_segment_HS.csv', row.names=FALSE)
write.csv(random_scenario_info$hikurangi, 'random_scenarios_kermadectonga2_hukurangi_segment_HS.csv', row.names=FALSE)
...
write.csv(random_scenarios_mean_curve, 
          'random_scenarios_kermadectonga2_logic_tree_mean_curve_HS.csv', row.names=FALSE)
# As above, for the "SUM-OF-SEGMENTED-MODELS"
set.seed(REPRODUCIBLE_SEED)
random_scenarios_segmented_union = ptha18$randomly_sample_scenarios_by_Mw_and_rate(
    (kt_tonga$HS_event_rates + kt_kermadec$HS_event_rates + kt_hikurangi$HS_event_rates),
    event_Mw,
    event_importance_weighted_sampling_probs=EVENT_IMPORTANCE_WEIGHTED_SAMPLING_PROBS,
    samples_per_Mw = samples_per_Mw_FUNCTION,
    mw_limits = c(min_mw_limit, max_mw_limit))
...
write.csv(random_scenarios_segmented_union, 
          'random_scenarios_kermadectonga2_logic_tree_segmented_union_HS.csv', row.names=FALSE)
