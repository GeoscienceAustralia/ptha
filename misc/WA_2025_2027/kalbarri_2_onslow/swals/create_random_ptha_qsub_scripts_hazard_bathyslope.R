...
raster_files = Sys.glob('../sources/hazard_bathyslope/random_*/scenario_initial_conditions/*.tif')
sourcezone_dirs = basename(dirname(dirname(raster_files)))
model_group_name = 'ptha18-kalbarri2onslow-hazard_bathyslope'
model_names = paste0(model_group_name, '/', sourcezone_dirs, '/',
    'ptha18_random_scenarios_', gsub('.tif', '', basename(raster_files), fixed=TRUE))

jobs_per_qsub_file = 5 # Run this many models in a single qsub file (or less), to stay within NCI walltime limits
...
with(model_run_creation_env, {
    ...
    multidomain_design_control_namelist = 'multidomain_design_control/multidomain_kalbarri2onslow_20251218_hazard.nml'
