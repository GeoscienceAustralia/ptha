...
* Qsub scripts in [./run_scripts](./run_scripts) 
   * **Excludes some historical test cases that were run with earlier iterations of the models.**
...
3. Create the PBS scripts to run relevant scenarios in [../sources/](../sources/), and then submit them
  ...
  * Tests of the tidal adjustment technique run with `Rscript create_random_ptha_qsub_scripts_hazard_bathyslope.R`
...
7. Compress the model output tar files (basename like `RUN_(DATETIME_STAMP).tar`) using `post_process/bzip2_some_files.R`.
