...
subroutine setup_elevation(domain, all_dx_md, close_input_files)
    ...
    !write(log_output_unit, *) 'SMOOTHING ELEVATION TWICE (FIXME)'
    ...
    if(close_input_files .and. allocated(open_estuary_entrances_polygons%polyvalue)) &
        deallocate(open_estuary_entrances_polygons%polyvalue)
    ...
    if(tidal_adjustment_file_subtract_from_elevation /= "") then
        ...
        do j = 1, domain%nx(2)
            ...
            where(adjustment > raster_na_below_limit) &
                ...
                ! NB: In the Gladstone model the above code used addition of a negative tidal raster,
