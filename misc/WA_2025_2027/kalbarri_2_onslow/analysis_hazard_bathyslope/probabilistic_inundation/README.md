...
```
Rscript compute_mean_exrate_upper_CI.R ptha18-kalbarri2onslow-hazard_bathyslope/highres_depth_with_variance depth 0.001
```
5. Go into the new folder in `ptha18-kalbarri2onslow-hazard_bathyslope/highres_depth_with_variance` that contains the `sum_of_source_zones` results. Then manually make VRT files for the important ones using a command like the one below.
```
...
Rscript compute_sum_of_percentiles.R ptha18-kalbarri2onslow-hazard_bathyslope/highres_depth_epistemic_uncertainty/ 84 depth 0.001
Rscript compute_sum_of_percentiles.R ptha18-kalbarri2onslow-hazard_bathyslope/highres_depth_epistemic_uncertainty/ 16 depth 0.001

# Then go to the folders inside
# `ptha18-kalbarri2onslow-hazard_bathyslope/highres_depth_epistemic_uncertainty/84pc`
