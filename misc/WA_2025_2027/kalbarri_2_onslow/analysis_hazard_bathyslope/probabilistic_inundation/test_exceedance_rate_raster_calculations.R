...
comparison_calculation_RDS = '../max_stage_at_a_point/nonlinear_model_curves_and_ptha18_curves_113.65335_-21.50723.RDS'

source_zone = 'sunda2'
# Tarred models
tarred_multidomain_dirs = Sys.glob(
    '../../swals/OUTPUTS/ptha18-kalbarri2onslow-hazard_bathyslope/random_sunda2/ptha*/RUN*.tar.bz2')

# Logic tree mean scenarios
scenario_databases = list()
scenario_databases$logic_tree_mean_curve_HS = read.csv(
    '../../sources/hazard_bathyslope/random_sunda2/random_scenarios_sunda2_logic_tree_mean_curve_HS.csv')
...
domain_index = swals$domain_index_from_folder(
    basename(domain_containing_point$domain_dir))
...
#
make_raster = errc$compute_exceedance_rates_on_tile(
...
#    ../max_stage_at_point/
# 
if(abs(result - expected_result$exrate) < 9.0e-10){
    print('PASS')
}else{
    print('FAIL')
}
...
xnew = raster(paste0('test_dir_2/logic_tree_mean_curve_HS_domain_', domain_index, '_max_stage_domain__exceedance_rate_with_threshold_1.tif'))
xnew_var = raster(paste0('test_dir_2/logic_tree_mean_curve_HS_domain_', domain_index, '_max_stage_domain__variance_of__exceedance_rate_with_threshold_1.tif'))
