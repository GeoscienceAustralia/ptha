...
# Used to find the domain index containing a particular point
...
reference_raster_tar_file = Sys.glob('../../swals/OUTPUTS/ptha18-kalbarri2onslow-hazard_bathyslope/random_*/*/raster_output_files.tar')[1]
...
source_zone_modelled_tsunami_scenario_basedirs = list(
    'outerrisesunda' = '../../swals/OUTPUTS/ptha18-kalbarri2onslow-hazard_bathyslope/random_outerrisesunda/',
    'sunda2'         = '../../swals/OUTPUTS/ptha18-kalbarri2onslow-hazard_bathyslope/random_sunda2/')
...
get_scenario_metadata_from_md_base_dir<-function(MD_BASE_DIR){
    ...
    tarred_multidomain_dirs = Sys.glob(paste0(MD_BASE_DIR, c('/ptha18_*/RUN*.tar', '/ptha18_*/RUN*.tar.bz2'))) # Match BOTH tar and tar.bz2
    stopifnot(all(file.exists(raster_tar_files)) & 
    ...
    if(source_zone == 'outerrisesunda'){
        ...
        scenario_base = '../../sources/hazard_bathyslope/random_outerrisesunda/'
        ...
        scenario_base = '../../sources/hazard_bathyslope/random_sunda2/'
...
get_raster_name_stub_from_variable_name<-function(VARIABLE_NAME){
    if(VARIABLE_NAME == 'depth'){
        ...
        # Use of neg_max_stage is mainly useful for testing the min_stage calculations,
        # since it uses 'negation' (as for min_stage), and by definition should produce the negative
        # of the 'max_stage' results (making it easy to test).
        # BEWARE!! The negation is taken care of in other parts of the code.
        raster_name_stub = 'max_stage_domain_'
    }else{
