...
if(VARIABLE_NAME %in% c('min_stage', 'neg_max_stage')){
    stop(paste0('Code does not yet support variable ', 
        VARIABLE_NAME, 
        '. To see how to add support, look at the code to compute a threshold given an exrate ',
        ' and uncertainty percentile (by negating the variable before and after processing).'))
}
...
for(nm_i in all_source_names){
    all_source_rate_info[[nm_i]] = 
        euf$get_logic_tree_branch_mw_bin_rates_and_posterior_probs(
            nm_i, all_samples[[nm_i]], ptha18, ptha18_source_rate_env, mu_type)
}
