...
#' @param mu_type either 'constant' or 'variable_mu'
...
get_logic_tree_branch_mw_bin_rates_and_posterior_probs<-function(
    ...
    ptha18, ptha18_source_rate_env, mu_type='constant'){
    ...
    # Variable rigidity changes the posterior probabilities that should be used.
    if(mu_type == 'constant'){
        posterior_prob = all_branches$all_par_prob
    }else if(mu_type == 'variable_mu'){
        posterior_prob = all_branches$all_par_prob_with_Mw_error
    }else{
    ...
    return(list(
        ...
        logic_tree_branch_posterior_prob = posterior_prob,
...
make_all_pixel_data<-function(scenario_row_index, raster_tar_files, 
    source_zone, DOMAIN_INDEX, raster_name_stub, MC_CORES, MINIMUM_DEPTH='adaptive_minimum', 
    MAXIMUM_DEPTH='adaptive_maximum', negate=FALSE){
