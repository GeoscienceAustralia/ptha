...
mu_type = 'variable_mu' # 'constant' # Shear modulus model 
REPRODUCIBLE_SEED = 98765
SAMPLING_MW_LIMIT = 9.65 # Make this in-between Mm values in PTHA18 (7.2, 7.3, ....9.6, 9.7, 9.8)
TOTAL_SAMPLES = 450 # spread over 25 bins that are below Mw limit
SAMPLES_PER_BIN_BELOW_MW_LIMIT = TOTAL_SAMPLES/25
fraction_uniform_sampling = 0.25 # How much sampling effort to spend on uniform sampling (the rest spread over 2 non-uniform sampling efforts)
stage_seq = seq(0.1, 5, by=0.1) # Range for plotting
stage_threshold_1 = 0.75 # Optimise sampling partly for this threshold
stage_threshold_2 = 2.5 # Optimise sampling partly for this threshold
sampling_effort_plot_title= paste0('Optimal number of Sunda Arc thrust scenarios @ ', round(target_point[1], 3), ',', 
    round(target_point[2], 3), ' \n using variable_mu logic-tree mean scenario-frequency model.')
...
# % uniform, % weighted non-uniform, rounded to integer values
chosen_sampling_effort_IS = round(
    fraction_uniform_sampling*uniform_sampling_effort + 
    (1.0-fraction_uniform_sampling)*(average_nonuniform_sampling_effort_IS))
...
png('Stage_vs_exrate_SundaArc.png', width=9, height=8, units='in', res=300)
options(scipen=5)
