...
print('CHANGING INITIAL CONDITION FILES TO INCLUDE EFFECT OF SLOPING BATHYMETRY')

rename_initial_condition_files_to_account_for_sloping_bathy<-function(source_zone, initial_condition_file){
    # Replace the initial condition files with new files that account for sloping bathymetry in the vertical deformation.
    tmp = gsub('EQ_SOURCE', 'EQ_SOURCE_bathyslope2025', initial_condition_file)
    tmp_basename = gsub(source_zone, paste0(source_zone, '_vertical_inc_bathyslope_kajiura'), basename(tmp))
    return(paste0(dirname(tmp), '/', tmp_basename))
}
sz_HS_scenarios$unit_source_statistics$initial_condition_file = rename_initial_condition_files_to_account_for_sloping_bathy(
    source_zone, sz_HS_scenarios$unit_source_statistics$initial_condition_file)
...
if(is_on_nci){
    ...
    cache_dir = paste0('SOURCE_ZONES/', source_zone, '/EQ_SOURCE_bathyslope2025/Unit_source_data/', source_zone)
    dir.create(cache_dir, recursive=TRUE, showWarnings=FALSE)
    copy_worked = file.copy(sz_HS_scenarios$unit_source_statistics$initial_condition_file, cache_dir)
