...
# At this stage, there is no limitation limit from PTHA.
...
if(asfm$LOWER_AMBIENT_SEA_LEVEL_IN_POLYGONS){
    for(i in 1:length(asfm$LOWER_AMBIENT_SEA_LEVEL_ZONES)){
        # Special treatment for sites where we set the initial sea-level to a lower value
        lsl_sites = rasterize(asfm$LOWER_AMBIENT_SEA_LEVEL_ZONES[[i]]$polygon, jatwc_zones_all_scenarios, background=0)

        stopifnot(asfm$LOWER_AMBIENT_SEA_LEVEL_ZONES[[i]]$sealevel < AMBIENT_SEA_LEVEL) # The code logic demands this holds
        ...
        included_sites = included_sites | (
            (lsl_sites == 1) & 
            (jatwc_zones_all_scenarios >= (asfm$LOWER_AMBIENT_SEA_LEVEL_ZONES[[i]]$sealevel + WET_TOL)) & 
