...
The subfolder [../elevation/limit_elevation_maxima_polygons](../elevation/limit_elevation_maxima_polygons) is used to specify an upper limit to the elevation maxima in particular regions. This can be useful to enforce channels, etc.
...
## [../multidomain_design_B](../multidomain_design_B): 
...
* Some domains can be flagged to use a reduced finite-volume theta parameter. This is useful for stabilising models if specific domains are going unstable in a way that can be prevented with a bit more dissipation.
...
Limit the initial stage in particular polygons. This is mostly useful for
...
* Start by modifying the `model_multidomain_design_mod.f90` to mostly match what you want. 
  * Variables can be set at run time via two namelists. These are defined in an input file that is passed to the main program (e.g. `multidomain_design_control/my_model_setup.nml`).
...
* Check the other model outputs, e.g.
  * Is the correct timestepping method used? 
  * Are gauges in the right place? 
  * Does manning friction match expectations? 
  * Is the resolution of the outputs as expected?
...
* Most often the `global_dt` is limited by the shortest timestep on the coarsest domains. 
  * The nested domains can usually do local timestepping (finite volume schemes only)
...
* If the time in communication (`comms1`) is fairly equal among processes but remains a high fraction of the `evolve:` time, consider whether it is more efficient to:
  ...
  * Wait and run again. 
    * More than once I've found Gadi running slowly, with lots of time in `comms1`, yet the same model ran fast later without changes. Sometimes you end up on a problematic node, or the netwrk or disk are under lots of pressure.
...
Before running all scenarios at full resolution, consider doing a "dry run" with a 2x coarser model.
...
* Screen the log files (`post_process/check_log_files.R`) to detect more obvious instabilities.
* When simulating a few hundred scenarios, it isn't unusual for one or two to go unstable, even using the approach above. 
  * I have seen cases where the NCI node failed
    * These instances often led to the creation of various MPI debug files (not from SWALS), and I believe they represent rare problems with the NCI network.
  ...
  * Other problems have manifest as a sudden model blow-up (NaN) near a coarse-2-fine nesting boundary, without any precursor that something was wrong. 
    * They were practically solvable by changing minor details of the nesting interpolation (below) or, in case the instability was internal to a domain, by using a reduced FV theta parameter for the offending domains.

#### Fixing unstable models
Run in `debug` mode by running `./model_debug` instead of `./model_build`. Then search the log files for the word `overflow`, which will describe the domain and location where extreme flow was found (usually indicating a developing instability). Have a look at the final timestep netcdf outputs for that domain, and try to understand what is going on.
* Is the timestep too large?
* Is the model going unstable at the boundary of a nesting zone (in which case you might want to adjust the nesting)?
* Is the model going unstable in the interior of a domain (i.e. not nesting related) during extreme inundation? It might help to use a reduced FV theta value on that domain.
  ...
  * Beware that complications will arise if you want to combine results from models with different multidomain setups / grid sizes etc. It's better to avoid this by identifying problems with the models before doing large runs.
  * However, it can happen that you've successfully completed most runs with the chosen timestep, but a few are unstable. In that case, you can reduce the timestep of the failed runs by some integer factor, without causing other changes to the output grid geometry, by using `reduce_timestep_with_no_other_changes`.
