...
In general we employ two techniques to generate them, either by specifying a [specific elevation](fixed_height_lines) or by taking the [maximum elevation within a buffer zone](buffer_height_lines) for each point on the line. However, in this case only the former is actually used.
