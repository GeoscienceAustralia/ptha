...
#   Mode    TRUE 
# logical     369 
if(any(!all_checks$run_finished)){
    print(all_log_files[!all_checks$run_finished])
}
...
for(i in 1:N){
    ...
    if(is(match_OK, 'try-error')){
       print(paste0('Try error on i=', i, ' ', all_log_files[i]))
    }else if(!match_OK){
       stop(paste0('Failed match on i=', i, ' ', all_log_files[i]))
    }
}
dev.off()
