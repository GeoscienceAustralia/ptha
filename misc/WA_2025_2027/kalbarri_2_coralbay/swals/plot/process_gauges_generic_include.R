...
for(i in 1:length(event_data)){
    ...
    # Sometimes the nearest gauge is on land, and dry

    # Ensure the gauge is within some distance of the target. If it isn't, then we have no nearby
    # hazard point, so should move-on. 
    threshold_m = 1*1000 # 1km threshold
    if(distHaversine(
        cbind(gauges$lon[gi], gauges$lat[gi]), 
        matrix(c(target_gauges[i,1:2]), ncol=2, nrow=1)) > threshold_m){
        next
    }
