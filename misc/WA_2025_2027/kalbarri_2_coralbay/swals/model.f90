...
use multidomain_design_mod, only: global_ll, global_lw, global_dx_arcmin, &
    ...
    theta_fv_limiter, theta_fv_limiter_reduced, &
...
subroutine setup_domains_on_nesting_level_NL(md, NL, parent_domain_index, &
    ...
    use multidomain_design_mod, only: r_ll, r_ur, r_coarse, r_dpth, r_theta_fv
    ...
    real(dp) :: local_max_depth, local_approx_wave_speed, local_coslat, &
        local_approx_grid_size, local_approx_ts, reduce_theta_fv
    ...
    do j = 1, size(nesting_NL_domain_extents, 2)
        ...
        ! Set the finite-volume 'theta' parameter. If the multidomain metadata tags
        ! the domain to use a smaller theta, then 
        reduce_theta_fv = nesting_NL_domain_extents(r_theta_fv,j)
        if(reduce_theta_fv == 0.0_dp) then
            ! Regular theta
            md%domains(di)%theta = theta_fv_limiter(NL)
        else
            ! Reduced theta (must be specified in namelist)
            md%domains(di)%theta = theta_fv_limiter_reduced(NL)
        end if

        if(nesting_domain_use_dispersion(NL)) then
            md%domains(di)%use_dispersion = .true.
            ! In some cases we need fatter halos, particularly in 'finely resolved deep areas'
            md%domains(di)%minimum_nesting_layer_thickness = maxval([40, nint(4*local_max_depth/local_approx_grid_size)]) !40_ip
...
subroutine try_to_reduce_timestepping_refinement_factors(md, reduced_dispersive_nt_max)
    ...
    ! This may allow domains to take fewer sub-time-steps, speeding things up, particularly if dispersion is being used 
    ! (as SWALS does less adaptation of FV timestepping in that case). 
    ...
    real(dp), parameter :: minimum_celerity = sqrt(gravity * 100.0_dp)
        !! This provides a way to prevent 'excessively large' timesteps that in practice could have problems
        !! as depths increase.
    ...
    domain_loop: do j = 1, size(md%domains)
        ...
        if(potential_trf > trf) then
            ...
            ! there is no guarentee that the halos are wide enough for stability, but it might be OK if allowed_timestep is too conservative.
