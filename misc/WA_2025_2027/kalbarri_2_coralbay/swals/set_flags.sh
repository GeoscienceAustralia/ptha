...
if [ "$FLAG_TYPE" == "preprocessor" ]; then
    ...
    elif [[ "$TARGET" == "both_debug_and_old_nesting" ]]; then
        SWALS_PREPROCESSOR_FLAGS+=" -DTRACK_MULTIDOMAIN_STABILITY -DEVOLVE_TIMER -DOLD_PROCESS_DATA_TO_SEND_B4FEB22"
    fi
    ...
    elif [[ "$TARGET" == "GD_home_machine" ]]; then
        SWALS_FC_ARCH_FLAGS=""
    fi
