...
use multidomain_design_mod, only: &
    ...
    limit_elevation_maxima_polygons_values_file, &
    tidal_adjustment_file_subtract_from_elevation, &
...
private
public :: set_initial_conditions, rise_time, stage_file_is_raster, &
    ambient_sea_level, parse_inputs
...
character(len=charlen), allocatable :: input_elevation_files(:), input_manning_n_files(:)
    !! Elevation and Manning input raster filenames
type(multi_raster_type):: elevation_data, manning_data, tidal_adjustment, stage_data
    !! Objects to interpolate from elevation/manning/tidal adjustment/stage rasters

character(len=charlen), allocatable :: breakwall_csv_lon_lat_z(:), invert_csv_lon_lat_z(:)
    !! Filenames containing breakwall and invert x/y/z data
type(xyz_lines_type) :: breakwalls_forced, inverts_forced
    !! Type with lon/lat/elev representing the tops of breakwalls or channel inverts.
    !! These features are "burned" into the elevation so that on coarser
    !! grids, we don't accidently miss key flow features. 

type(polygons_values_type) :: override_initial_stage_polygons, limit_elevation_maxima_polygons
    !! Used to set the initial stage in polygons. This will override the 
    !! 'ambient_sea_level' and any static initial condition. 
    !! Also limit the maximum elevation inside estuary entrances (to open them)

character(len=charlen), allocatable :: unit_source_stage_rasts(:)
    !! Used to read one or more stage raster files for initial condition
real(dp), allocatable :: unit_source_slips(:), unit_source_start_times(:), unit_source_end_times(:)
    !! Variables for one or more stage files for initial condition
...
subroutine parse_inputs(stage_file, multidomain_design_namelists, &
    ...
    ! Sanity check on interpreting NA values in rasters
    if(raster_na_below_limit < (-0.99_dp * HUGE(1.0_dp))) then ! Somewhat arbitrary threshold here
        write(log_output_unit, *) 'raster_na_below_limit should be much greater than ( -HUGE(1.0_dp) ), so that' // &
            ' even if we add input data values to (-HUGE(1.0_dp)) the result is still < raster_na_below_limit'
        call generic_stop
    end if

end subroutine

subroutine set_initial_conditions(domain, stage_file, global_dt, all_dx_md, finalise_input_files)
    ...
    logical, optional, intent(in) :: finalise_input_files
        ! By setting this to .false. except on the last call to this routine, we can avoid excessive opening 
        ! and closing of input files, which can be faster (since the opening involves reading lots of metadata).

    logical:: close_input_files

    ! By default close input files every time (previous behaviour, will work even if slow)
    close_input_files = .true.
    if(present(finalise_input_files)) close_input_files = finalise_input_files

    ! Basic setting of elevation -- modified again later by the earthquake forcing.
    call setup_elevation(domain, all_dx_md, close_input_files)

    ! Set the earthquake initial condition [stage + elevation perturbation]
    call setup_stage_and_forcing(domain, stage_file, global_dt, close_input_files)

    ! Set the Manning coefficient
    call setup_manning(domain, close_input_files)

end subroutine

subroutine setup_manning(domain, close_input_files)
    !! Set the Manning coefficient for the domain
    !! Read from a raster file, otherwise set to default value.
    type(domain_type), intent(inout) :: domain
    logical, intent(in) :: close_input_files

    ! Coordinates for stage lookup
    real(dp), allocatable:: x(:), y(:)
    integer(ip) :: i, j
    character(len=charlen), parameter :: char_format = '(A)'
    real(dp):: default_manning
    ...
    ! Set the manning coefficient
    domain%manning_squared = -1.0_dp ! Negative value used to denote 'not filled' below
    if(len_trim(mannning_n_file_list_in_preference_order) > 0) then
        ...
        if(.not. allocated(input_manning_n_files)) then
            call read_character_file(mannning_n_file_list_in_preference_order, &
                input_manning_n_files, char_format)
        ...
        do j = 1, domain%nx(2)
            ...
            ! Read raw manning coefficient into domain%manning_squared
            call manning_data%get_xy(x, y, domain%manning_squared(:,j), &
                domain%nx(1), bilinear=1_ip, &
                na_below_limit=raster_na_below_limit)

            ! Square the manning coefficient outside of unset areas
            where (domain%manning_squared(:,j) > 0.0_dp) &
                domain%manning_squared(:,j) = domain%manning_squared(:,j)**2
        end do
    end if

    if(close_input_files .and. allocated(manning_data%raster_files)) then
        ! Cleanup
        deallocate(input_manning_n_files)
        call manning_data%finalise()
    end if
          
    ! Default Manning coefficient
    default_manning = merge(default_manning_staggered_grid, &
        default_manning_non_staggered_grid, domain%is_staggered_grid)
    do j = 1, domain%nx(2)
        where(domain%manning_squared(:,j) < 0.0_dp) &
            domain%manning_squared(:,j) = default_manning**2
    end do

    ! Sanity check that we don't have negative values
    if(minval(domain%manning_squared) < 0.0_dp) then
        write(log_output_unit, *) 'Negative squared Manning coefficient --> problem in model setup'
        call generic_stop
    end if
    ...
    if(any(domain%manning_squared /= domain%manning_squared)) then
        ...
        do i = 1, domain%nx(1)
            if(domain%manning_squared(i,j) /= domain%manning_squared(i,j)) &
                write(log_output_unit,*) '  ', x(i), y(i)
        end do
        call generic_stop
    end if
...
subroutine setup_elevation(domain, all_dx_md, close_input_files)
    ...
    logical, intent(in) :: close_input_files
        !! Can be used to avoid reinitialising input data too often
    ...
    do j = 1, domain%nx(2)
        ...
        if(any(domain%U(:,j,ELV) /= domain%U(:,j,ELV))) then
            ...
            do i = 1, domain%nx(1)
                if(domain%U(i,j,ELV) /= domain%U(i,j,ELV)) &
                    write(log_output_unit,*) '  ', x(i), y(i)
            end do
            call generic_stop
        end if
    ...
    if(smooth_elevation_along_nesting_boundaries) then
        ...
        ! Site locations and smoothing radius chosen from inspection of nesting artefacts.

        ! These sites are on a coarse-ish nesting boundary far from the area of
        ! interest, and can develop high speeds in extreme inundation cases. Probably
        ! not important but better not to have it.
        call domain%smooth_elevation_near_point(&
            number_of_9pt_smooths = 1_ip, &
            pt = [118.000_dp, -20.550_dp], &
            smooth_region_radius_meters = 300.0_dp, &
            transition_region_radius_meters = 500.0_dp)
        call domain%smooth_elevation_near_point(&
            number_of_9pt_smooths = 1_ip, &
            pt = [118.000_dp, -20.568_dp], &
            smooth_region_radius_meters = 300.0_dp, &
            transition_region_radius_meters = 500.0_dp)

        ! This site is on Dirk Hartog Island, just north of Steep Point, at a nesting boundary.
        ! Inundates in extreme cases only, can develop high speeds.
        call domain%smooth_elevation_near_point(&
            number_of_9pt_smooths = 1_ip, &
            pt = [113.18386_dp, -26.11111_dp], &
            smooth_region_radius_meters = 60.0_dp, &
            transition_region_radius_meters = 100.0_dp)
    ...
    !
    ! Below we enforce breakwalls/inverts/other elevation limits. Deliberately done after smoothing
    ! since we don't want these to be smoothed.
    !

    call setup_breakwalls_and_inverts(domain, close_input_files)

    write(log_output_unit, *) 'Elevation ranges after setting breakwalls and inverts:'
    write(log_output_unit, *) minval(domain%U(:,:,ELV)), maxval(domain%U(:,:,ELV))

    if(close_input_files) then
        ! Cleanup the elevation data structures
        if(allocated(input_elevation_files)) deallocate(input_elevation_files)
        if(allocated(elevation_data%raster_files)) call elevation_data%finalise()
    end if

    !
    ! Optionally limit the maximum elevation in user-provided polygons
    !
    if(limit_elevation_maxima_polygons_values_file /= "") then

        ! Setup the polygons_values data structure the first time this is called
        if(.not. allocated(limit_elevation_maxima_polygons%polyvalue)) then

            call limit_elevation_maxima_polygons%setup_from_csv_with_file_value_columns(&
                limit_elevation_maxima_polygons_values_file, &
        ...
        call limit_elevation_maxima_polygons%burn_into_grid(domain%U(:,:,ELV), &
    ...
    if(close_input_files .and. allocated(limit_elevation_maxima_polygons%polyvalue)) &
        deallocate(limit_elevation_maxima_polygons%polyvalue)

    !
    ! Adjust the elevation to represent tides. 
    !
    if(tidal_adjustment_file_subtract_from_elevation /= "") then

        if(.not. allocated(tidal_adjustment%raster_files)) &
            call tidal_adjustment%initialise([tidal_adjustment_file_subtract_from_elevation])

        allocate(adjustment(domain%nx(1))); adjustment = 0.0_dp

        ! Set the adjustment row-by-row (less memory than all at once).
        do j = 1, domain%nx(2)
            y = domain%y(j)
            call tidal_adjustment%get_xy(x, y, adjustment, domain%nx(1), &
                bilinear=1_ip, na_below_limit=raster_na_below_limit)
            ! Areas outside the tidal_adjustment raster will have 
            !     adjustment = -HUGE(1.0_dp)
            ! which is much less than raster_na_below_limit
            where(adjustment > raster_na_below_limit) &
                domain%U(:,j,ELV) = domain%U(:,j,ELV) - adjustment 
                ! NB: In the Gladstone model the equivalent code used addition of a negative tidal raster,
                !     whereas here we subtract a positive tidal raster.
            ...
            if(any(domain%U(:,j,ELV) < raster_na_below_limit)) then
                ...
                do i = 1, domain%nx(1)
                    if(domain%U(i,j,ELV) < raster_na_below_limit) &
                        write(log_output_unit,*) '  ', x(i), y(i)
                end do
    ...
    write(log_output_unit, *) 'Elevation ranges after tidal adjustment:'
    write(log_output_unit, *) minval(domain%U(:,:,ELV)), maxval(domain%U(:,:,ELV))

    if(close_input_files) then
        ! Cleanup the input data structures
        if(allocated(tidal_adjustment%raster_files)) call tidal_adjustment%finalise()
    end if
    ...
    if( any(domain%timestepping_method == [character(len=charlen) :: &
        ...
        ! sends to the nonlinear domain one can have wet-dry issues. Note
        ! these timestepping methods cannot model inundation.
        where((domain%U(:,:,ELV) < ambient_sea_level) .and. &
...
subroutine setup_breakwalls_and_inverts(domain, close_input_files)
    !! Enforce breakwalls (i.e. elevation >= breakwall) and inverts 
    !! (i.e. elevation <= invert) in the domain's elevation grid, i.e.
    !! domain%U(:,:,ELV)
    type(domain_type), intent(inout) :: domain
        !! domain%U(:,:,ELV) is modified
    logical, intent(in) :: close_input_files
        !! .TRUE. will cause deallocation of the breakwalls/inverts

    character(len=charlen), parameter :: char_format = '(A)'

    if(breakwalls_file_list /= "" .and. (.not. allocated(breakwalls_forced%lines))) then
        ...
        call breakwalls_forced%read_from_csv(breakwall_csv_lon_lat_z, skip_header=1_ip)
    ...
    if(allocated(breakwalls_forced%lines)) &
        call breakwalls_forced%burn_into_grid(domain%U(:,:,ELV), &
            domain%lower_left, (domain%lower_left + domain%lw), burn_type='max')

    if(inverts_file_list /= "" .and. (.not. allocated(inverts_forced%lines))) then
        ! On the first call, setup the inverts_forced object.

        ! Read the invert files
        call read_character_file(inverts_file_list, invert_csv_lon_lat_z, char_format)

        ! Setup inverts interpolation object
        call inverts_forced%read_from_csv(invert_csv_lon_lat_z, skip_header=1_ip)
    ...
    if(close_input_files) then
        ...
        if(allocated(breakwall_csv_lon_lat_z)) then
            deallocate(breakwall_csv_lon_lat_z)
            deallocate(breakwalls_forced%lines)
        end if
        if(allocated(invert_csv_lon_lat_z)) then
            deallocate(invert_csv_lon_lat_z)
            deallocate(inverts_forced%lines)
        end if
    end if

end subroutine

subroutine setup_stage_and_forcing(domain, stage_file, global_dt, close_input_files)
    ...
    real(dp), intent(in) :: global_dt
        ...
        !! time-step].
    logical, intent(in) :: close_input_files
        !! .TRUE. will cause deallocation of stage input data
    ...
    if(stage_file /= "") then
        if(stage_file_is_raster .and. rise_time == 0.0_dp) then
            ...
            if(.not. allocated(stage_data%raster_files)) &
            ...
            do j = 1, domain%nx(2)
                ...
                where(domain%U(:,j,STG) < (raster_na_below_limit) ) domain%U(:,j,STG) = 0.0_dp
            end do
            deallocate(x,y)
            if(close_input_files) call stage_data%finalise()
            ...
            if(.not. allocated(unit_source_stage_rasts)) then            
                ...
                if(stage_file_is_raster) then
                    ...
                    n = 1
                    unit_source_stage_rasts = [stage_file]
                    unit_source_start_times = [global_dt] ! Avoid forcing in first time-step
                    unit_source_end_times   = [global_dt + rise_time]! Avoid forcing in first time-step
                    unit_source_slips       = [1.0_dp] ! Equivalent to using the raw raster values
                else
                    ! Read the data to perform the unit-source summation
                    open(newunit=fid, file=stage_file)
                    n = count_file_lines(fid) - csv_header_size
                    allocate(unit_source_stage_rasts(n), unit_source_slips(n), &
                        unit_source_start_times(n), unit_source_end_times(n))
                    ! Skip header
                    ...
                    ! Read data
                    do j = 1, n
                        read(fid, *) unit_source_stage_rasts(j), unit_source_slips(j), &
                            unit_source_start_times(j), unit_source_end_times(j)
                    end do
                    close(fid)
                    unit_source_start_times = unit_source_start_times + global_dt ! Avoid forcing in first time-step
                    unit_source_end_times = unit_source_end_times + global_dt ! Avoid forcing in first time-step
                end if
            end if

            do j = 1, size(unit_source_stage_rasts)
                call setup_forcing_with_rise_time(domain, unit_source_stage_rasts(j), &
                    unit_source_slips(j), unit_source_start_times(j), unit_source_end_times(j))
            end do

            if(close_input_files) deallocate(unit_source_stage_rasts, unit_source_slips, &
                unit_source_start_times, unit_source_end_times)
        end if
    end if
    ...
    if(close_input_files .and. allocated(override_initial_stage_polygons%polyvalue)) &
        deallocate(override_initial_stage_polygons%polyvalue)

    ! Alway need stage >= elevation.
    domain%U(:,:,STG) = max(domain%U(:,:,STG), domain%U(:,:,ELV) + (minimum_allowed_depth/100.0_dp) )
...
subroutine setup_forcing_with_rise_time(domain, stage_file, slip, start_time, end_time)
    ...
    real(dp), intent(in) :: slip
        !! Multiply the unit source perturbation by this value to match the desired slip. 
        !! A value of 1.0 will use the raster unchanged.
    ...
    k0 = STG
    k1 = ELV ! Requires using solvers that support elevation forcing 
             ! (and prior setup for solvers with optional elevation forcing)
