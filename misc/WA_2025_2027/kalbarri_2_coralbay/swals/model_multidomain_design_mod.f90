...
real(dp) ::  global_ll(2) = [90.0_dp, -40.0_dp]

! Length/width of multidomain in degrees lon,lat
real(dp) :: global_lw(2) = [135.0_dp - 90.0_dp, 0.0_dp - (-40.0_dp)]
...
! not, it is adjusted to a nearby value.
...
real(dp) ::  global_dt = 1.6_dp ! * global_dx_arcmin

! Optionally replace the call to md%evolve_one_step(global_dt) with
! this many calls with shorter steps. This is useful if we discover
! a model went unstable due to the timestep being too large, and we
! want to reduce the timestep without any other changes to the domains
! (since other changes might complicate post-processing).
integer(ip) :: reduce_timestep_with_no_other_changes = 1_ip

! Final time (for non-test runs) in seconds. Start time is always 0.0
real(dp) ::  final_time_full_runs = 6.0_dp * 3600.0_dp
! Final time for test runs (e.g. used for load balancing or simple checks)
real(dp) ::  final_time_test_runs = 120.0_dp !360.0_dp

! Manning friction used on staggered grid domains (typically offshore)
real(dp), parameter :: default_manning_staggered_grid = 0.030_dp
! Manning friction used on cell centred grid domains (typically nearshore)
! (beware possible slowdown if not parameter). Beware there is no communication
! of Manning friction between domains, so if the default_manning differs
! on staggered/non-staggered grids, then we could have overlapping domains
! with inconsistent friction (...maybe not a problem, but....).
real(dp), parameter :: default_manning_non_staggered_grid = 0.030_dp

! Manning friction file names listed in priority order.
character(len=charlen) :: mannning_n_file_list_in_preference_order = ""
...
! Limit the maximum elevation in user provided polygons. If non-empty, this
...
!     ....
! where files like "full_path_to_polygon_file_1.csv" contain 2 columns and a single
! header line.
! The final elevation will not exceed the specified value inside the polygon.
character(len=charlen) :: limit_elevation_maxima_polygons_values_file = &
    "../elevation/limit_elevation_maxima_polygons/limit_elevation_polygons_values.csv"

! Optionally adjust the elevation from a raster to emulate variations in the tide.
! This raster will be SUBTRACTED from the elevation (different to Gladstone model)
character(len=charlen) :: tidal_adjustment_file_subtract_from_elevation = ""
...
!     full_path_to_polygon_file_2.csv, 1.2
!     ....
! where files like "full_path_to_polygon_file_1.csv" contain 2 columns and a
! single header line.
character(len=charlen) :: override_initial_stage_polygons_values_file = ""

! Workaround for raster missing data.
! Many rasters denote NA with a large negative value, but the NA value often becomes
...
! large negative threshold as NA (so long as these are never "genuine" values).
! The threshold must be significantly greater than (-HUGE(1.0_dp)), so we can add data
! values to (-HUGE(1.0_dp)) and still have a result less than this
...
character(len=charlen) :: swals_point_gauge_file = ""
...
character(len=charlen) :: nontemporal_grids_to_store(9) = ""
...
!    level "nesting_level_parent_index(i)" (by default, this equals "i-1")
! 3. The default grid-size of nesting level 'i' domains is an integer divisor
!    of the default grid-size on nesting_level_parent_index(i). The integer is specified
!    by dx_refinement_factors(i).
!
...
! Use dispersion (.true.) or not (.false.) on each nesting level separately
logical, allocatable :: nesting_domain_use_dispersion(:)
...
! Negative values imply no coarsening will be used (always true for the global domain)
integer(ip), allocatable :: coarsening_factors(:)

! Grid refinement is determined with reference to the an earlier nesting level.
! Typically, we would have nesting_level_parent_index = [0, 1, 2, 3, ...], where
! the first entry (0 here) is never used.
! For more complex nesting designs, we may want to modify this default. For
! example, consider a model with 120 m outer grid, with two different nested
! grids with cell sizes of 30 m and 40 m. These could both nest with the outer
! grid as a parent, but not with each other. In that case, we woul have nesting
! level parent index = [0, 1, 1].
integer(ip), allocatable :: nesting_level_parent_index(:)

! If 'rk2' or 'midpoint' timestepping is used, this controls the 'theta' limiter parameter.
...
! Domains can be flagged to use a reduced theta limiter parameter at the
! multidomain design. This can help with making models stable.
real(dp), allocatable :: theta_fv_limiter_reduced(:)
...
!     4,
!     3, 3
! The above would split domain 1 into 4 pieces, and domains 3 and 5 into 2
...
! file with more equal work between processes, using the R script "load_balance_script.R"
!
...
integer(ip), parameter :: &
    ...
    r_theta_fv = 8_ip         ! Index of "reduce FV theta parameter?" (0.0 = No, anything else = Yes)
...
namelist /MULTIDOMAIN_NESTING_PROPERTIES/ &
    ...
    coarsening_factors, &
    nesting_level_parent_index, &
    theta_fv_limiter, &
    theta_fv_limiter_reduced, &
    load_balance_file
...
subroutine read_multidomain_design_variables(multidomain_design_namelist_file)
    ...
    allocate(nesting_domain_extents_file(NNL), &
        nesting_domain_timestepping_method(NNL), nesting_domain_use_dispersion(NNL), &
        dx_refinement_factors(NNL), &
        coarsening_factors(NNL), nesting_level_parent_index(NNL), theta_fv_limiter(NNL))
    ...
    nesting_level_parent_index = [(i-1, i=1, NNL)]
    theta_fv_limiter = 4.0_dp ! Non-TVD limiting
    !theta_fv_limiter = 1.6_dp ! TVD limiting
    theta_fv_limiter_reduced = theta_fv_limiter ! By default do not change FV limiting
    nesting_domain_timestepping_method(1) = "leapfrog_linear_plus_nonlinear_friction"
    nesting_domain_timestepping_method(2:NNL) = "midpoint"
    nesting_domain_use_dispersion(1:NNL) = .false.
...
subroutine set_seconds_before_evolve(stage_file, run_type, seconds_before_evolve)
    ...
    ! See ../multidomain_design/ptha18_seconds_before_evolve.log for examples.
    character(len=charlen), intent(in) :: &
        stage_file, & !! filename with tsunami initial condition
        run_type !! either 'test' or 'test_load_balance' or 'full'. Only the latter uses seconds_before_evolve
    real(dp), intent(inout) :: seconds_before_evolve

    seconds_before_evolve = 0.0_dp

    !if(index(run_type, 'test') /= 0) then
    !    ! Runs of type 'test' or 'test_load_balance' always evolve straight away
    !    seconds_before_evolve = 0.0_dp
    !else if (index(stage_file, 'Tohoku2011') /= 0) then
    !    ! Tohoku 2011 source inversion
    !    seconds_before_evolve = 3600 * 6.5_dp
    !else if(index(stage_file, 'random_southamerica') /= 0) then
    !    ! South American ptha18 source
    !    ! flow |PTHA_{100 sampled scenarios}| < 0.0001 till 12.98335 hours
    !    ! flow |PTHA_{100 sampled scenarios}| < 0.00001 till 12.53341 hours
    !    seconds_before_evolve = 3600 * 10.0_dp
    !else if(index(stage_file, 'random_alaskaaleutians') /= 0) then
    !    ! Alaska-aleutians ptha18 source.
    !    ! flow |PTHA_{100 sampled scenarios 1}| < 0.0001 till 10.15029 hours.
    !    ! flow |PTHA_{100 sampled scenarios 2}| < 0.00001 till 9.942008 hours
    !    ! flow |PTHA_{100 sampled scenarios 3}| < 0.000001 till 9.916713 hours
    !    seconds_before_evolve = 3600 * 8.0_dp
    !else if(index(stage_file, 'random_kurilsjapan') /= 0) then
    !    ! Kurils-Japan ptha18 source
    !    ! flow |PTHA_{100 sampled scenarios}| < 0.0001 till 8.141748 hours
    !    ! flow |PTHA_{100 sampled scenarios}| < 0.00001 till 7.783849 hours
    !    ! flow |PTHA_{100 sampled scenarios}| < 0.000001 till 7.725186 hours
    !    seconds_before_evolve = 3600 * 6.5_dp
    !end if
end subroutine
