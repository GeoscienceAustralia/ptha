...
if(asfm$use_unadjusted_elevation_to_compute_jatwc_H_sample_points){
    scenario_elevation_raster_file = normalizePath(asfm$scenario_elevation_raster_file_NO_TIDAL_ADJUSTMENT)
}else{
    scenario_elevation_raster_file = normalizePath(asfm$scenario_elevation_raster_file_SEEN_BY_MODEL)
}
...
get_max_stage_percentile_in_zone<-function(scenario_max_stage, sample_points, prob=0.95){
    ...
    # For wet/dry checks we need to use the model's elevation (i.e. tidally adjusted). This may
    # not be the same as the elevation used to define "sample_points", since for the point location we just
    # want to mimic the H^95 statistic as used in JATWC.
    sample_elev = extract(raster(asfm$scenario_elevation_raster_file_SEEN_BY_MODEL), sample_points[,1:2])
