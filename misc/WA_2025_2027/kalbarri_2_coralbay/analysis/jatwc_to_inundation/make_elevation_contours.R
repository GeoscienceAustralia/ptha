#
# Make elevation contours in a given warning zone - NO TIDAL ADJUSTMENT
#
source('make_vrt.R')
library(terra)

# Use the model PTHA elevation rasters -- note only a subset of these will be in our warning zone.
ptha_elevation_rasts = Sys.glob('elevation_in_model_no_tidal_adjustment/elevation0*.tif')
...
zone_files_no_threat = Sys.glob(paste0('Inundation_zones/', ATWS_Zone_name_nospace, '/no_threat_max_stage*.tif'))
