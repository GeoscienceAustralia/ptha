...
* [depth_vs_exrate_at_gauge.R](depth_vs_exrate_at_gauge.R) is used to compute the exceedance-rates of maximum depth (and maximum-stage) at two sites; the Parliament site (for application), and a PTHA18 point (to compare results with the original PTHA18 study). It uses the gauge-outputs store by SWALS, so can be adapted for other sites where there are gauges, but it cannot be used for abtrary points. This is useful for testing (to confirm we get almost the same result from the independent raster-based calculations at that site as implemented in [probabilistic_inundation.R](probabilistic_inundation.R)). Also the outputs themselves are useful -- as the calculation only involves a single gauge, it is relatively fast. See further discussion in the script comments. The code is run with the following commandline calls: `Rscript depth_vs_exrate_at_gauge.R parliament` and `Rscript depth_vs_exrate_at_gauge.R 'ptha18_point_3458.3' `. 
