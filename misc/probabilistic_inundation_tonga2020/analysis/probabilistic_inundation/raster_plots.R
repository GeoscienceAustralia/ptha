...
tonga_coast = readOGR('../../elevation/Tonga_coast/Tonga_coast_nearlon180.shp', 'Tonga_coast_nearlon180')

# We may need to create the 'merged' rasters that add 0.5*(segmented + unsegmented source representations)
# This function does that
make_combined_segunseg_tifs<-function(){
    # All the rasters with exceedance-rate [given depth-threshold and domain] for this run_series
    all_rast_files = Sys.glob(paste0(run_series_name, '/*.tif'))

    # Extract key information from the filename
    all_rasts_domain_index_and_threshold = unlist(lapply(strsplit(basename(all_rast_files), '_domain_'), 
                                           f<-function(x) x[2]))
    # We need to add unsegmented and segmented source representations where
    # there domain index and threshold match
    unique_all_rasts_domain_i_and_t = unique(all_rasts_domain_index_and_threshold)
    ...
    for(i in 1:length(unique_all_rasts_domain_i_and_t)){
        ...
        k = which(all_rasts_domain_index_and_threshold == unique_all_rasts_domain_i_and_t[i])
        ...
        # To combine segmented and unsegmented we do ( 0.5*unseg + 0.5*(sum_of_segments) )
        # which is the same as sum(0.5 * each_raster)
        summed_rast = local_rasts[[1]] * 0
        for(j in 1:length(local_rasts)) summed_rast = summed_rast + 0.5*local_rasts[[j]]

        # Easiest to change 0 back to NA so we don't have zeros in halo regions.
        summed_rast[summed_rast == 0] = NA

        output_file = paste0(run_series_name, '/combined_segunseg_DOMAIN_', unique_all_rasts_domain_i_and_t[i])
        writeRaster(summed_rast, output_file, options=c("COMPRESS=DEFLATE"), overwrite=TRUE)
    }
...
# All the rasters with exceedance-rate [given depth-threshold and domain] for this run_series
all_rast_files = Sys.glob(paste0(run_series_name, '/combined_segunseg_DOMAIN_*.tif'))
if(length(all_rast_files) == 0){
    # We haven't yet made the combined segmented/unsegmented files
    # Do it now.
...
all_rasts_source_model = unlist(lapply(strsplit(basename(all_rast_files), '_DOMAIN_'), 
                                f<-function(x) x[1]))
stopifnot(all(all_rasts_source_model == 'combined_segunseg'))

## The domain index [1-7]
all_rasts_domain_index = as.numeric(unlist(lapply(strsplit(basename(all_rast_files), '_DOMAIN_'), 
                                                  f<-function(x) strsplit(x[2], '_')[[1]][1])))
## The depth threshold [0.001, 0.1, .... ]
all_rasts_threshold = as.numeric(unlist(lapply(strsplit(basename(all_rast_files), '_threshold_'), 
...
#colz_unique = colorRampPalette(c('lightblue', 'darkblue', 'orange', 'red', 'purple', 'black'))(n)
...
colz_unique_labels = c(paste0(unique_depth_thresholds[1:(n-1)], ' - ', unique_depth_thresholds[2:n]),
...
plot_all_rasters<-function(threshold_exrate=target_exrate, xlim=c(184.62, 185), ylim=c(-21.25, -21.05)){
...
legend('bottomleft', colz_unique_labels[-1], fill=colz_unique[-1], cex=1.3, title='Depth (m)')
title('Tsunami inundation depth: 2% chance of exceeding in 50 years', cex.main=1.6)
dev.off()
