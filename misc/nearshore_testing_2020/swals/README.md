...
* Files like `load_balance_files/load_balance_XXXXX.txt` -- These files tell SWALS how to split the domains for distributed memory parallel runs, in a way that is efficient with our machine, software and compiler options. 
    ...
    - Coarray-images are equivalent to mpi-ranks; except that `coarray-image-index = mpi-rank + 1`. Here we actually compiled the code to use MPI, rather than Fortran coarrays. However SWALS was originally written to use coarrays, and so the code often uses coarray concepts when referring to the distributed-memory parallel implementation. 
