...
# example from 
# Piatanesi, A. and Lorito, S. 2007 Rupture Process of the 2004 Sumatra–Andaman Earthquake from Tsunami Waveform Inversion Bulletin of the Seismological Society of America, 97, S223–S231
...
rp = read.csv('rupture_parameters.csv', colClasses='numeric')

#
# Input parameters -- rupture geometry
#
#
lat = rp$lat # rupture location in DEGRESS N, WGS84

lon = rp$lon # rupture location in DEGREES E, WGS84 
width = rp$width # rupture width in km
length = rp$length # rupture length in km
strike = rp$strike # Strike in degrees clockwise from N
dip = rp$dip # Dip
depth = rp$topdepth  # Depth at upper edge
rake = rp$rake # Degrees -- rupture rake
slip = rp$slip
rupture_name   =    as.character(1:length(lon)) # A name, which will appear in the filename
rupture_basename = 'Piatanesi_sumatra2004_sources' # Name at the start of output files
make_plot = FALSE

if(make_plot){
    # Quick check that the slip matches the picture in the paper
    plot(lon, lat, col=colorRampPalette(c('white', 'yellow', 'orange', 'red', 'black'))(341)[floor(slip*10)+1], asp=1, pch=19, cex=2)
    points(lon, lat, cex=2)
}

# lon/lat denote the southernmost point on the deepest edge. Move it
# along-strike to the 'middle deepest'.
