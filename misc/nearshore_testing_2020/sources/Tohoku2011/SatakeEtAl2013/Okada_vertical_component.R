...
# example from 
# Satake, K.; Fujii, Y.; Harada, T. & Namegaya, Y. Time and Space Distribution
# of Coseismic Slip of the 2011 Tohoku Earthquake as Inferred from Tsunami
# Waveform Data Bulletin of the Seismological Society of America, 2013, 103,
# 1473-1492
...
mydat = read.csv('satake13_data.csv')
#
...
width = c(rep(25, 22), rep(50, 33))  # rupture width in km
length = rep(50, 55) # rupture length in km
strike = rep(193, 55) # Strike in degrees clockwise from N
dip = c(rep(8, 11), rep(8, 11), rep(10, 11), rep(12, 11), rep(16, 11)) # Dip
depth = c(rep(0, 11), rep(3.5, 11), rep(7, 11), rep(15.6, 11), rep(26, 11)) # Depth at upper edge
rake  =  rep(81, 55) # Degrees -- rupture rake
slip = mydat$total
rupture_name   = mydat$name # A name, which will appear in the filename
rupture_basename = 'Satake_Tohoku11_sources' # Name at the start of output files
make_plot = FALSE

# Convert lon/lat to top-edge-middle
...
if(make_plot){
    ...
    plot(lon, lat, col=colorRampPalette(c('white', 'yellow', 'orange', 'red', 'black'))(70)[floor(slip)+1], asp=1, pch=19, cex=2)
