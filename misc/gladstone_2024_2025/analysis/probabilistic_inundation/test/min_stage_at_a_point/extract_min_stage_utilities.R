...
# Get the min-stage at the desired point from a single file.
get_min_stage_at_target_point<-function(tarred_raster_file, raster_filename, XIND, YIND, target_point, coordinate_errtol){
...
plot_tsunami_minima_in_nonlinear_model_and_PTHA18<-function(target_point, scenario_min_stages, dir_name){
    ...
    #
    sourceID = as.integer(as.factor(scenario_min_stages$nonlinear_source_name))
    ...
    source_name_ID = aggregate(sourceID, 
        by=list(source_name=scenario_min_stages$nonlinear_source_name), 
        function(x) x[1])

    out_file = paste0(dir_name, '/Tsunami_minima_in_PTHA18_and_Nonlinear_Model_', target_point[1], '_', target_point[2], '.png')
    png(out_file, width=12, height=9, units='in', res=300)
    plot(scenario_min_stages$ptha18_min_stage, scenario_min_stages$nonlinear_min_stage_minus_MSL,
         xlab='PTHA18 max-stage (36hours, linear frictionless model)',
         ylab='Highres model -min-stage',
         cex.lab=1.5, cex.axis=1.5, pch=19, asp=1,
         col=COLZ[sourceID])
    grid(col='orange')
    abline(0, 1, col='red')
    abline(0, median(scenario_min_stages$nonlinear_min_stage_minus_MSL/scenario_min_stages$ptha18_min_stage), col='blue')
    title(main='Tsunami minima in PTHA18 vs High-resolution model',
        cex.main=1.7)
    legend('bottomright', source_name_ID$source_name, pch=19, col=COLZ[source_name_ID$x], bty='n') 
    dev.off()

    # Another way of displaying this
    out_file = paste0(dir_name, '/Tsunami_minima_in_PTHA18_and_Nonlinear_Model_relative_', target_point[1], '_', target_point[2], '.png')
    mean_min_stage = scenario_min_stages$ptha18_min_stage 
    diff_min_stage = (scenario_min_stages$ptha18_min_stage - scenario_min_stages$nonlinear_min_stage_minus_MSL)
    reldiff =  diff_min_stage/mean_min_stage
    png(out_file, width=12, height=9, units='in', res=300)
    scatter.smooth(mean_min_stage, reldiff, xlab='PTHA18 max-stage',
...
# min-stages, which is useful for testing since in this situation the
...
plot_logic_tree_mean_exrate_curves_from_nonlinear_model_and_PTHA18<-function(
    ...
    ptha18_curves, ptha18_combined_curve, EXRATE_PLOT_YLIM, dir_name){

    # Plot the nonlinear model exceedance rates for each source zone.
    out_file = paste0(dir_name, '/Exceedance_Rates_in_PTHA18_and_Nonlinear_Model_', 
    ...
    for(nm in names(nonlinear_model_curves)){
        plot(ptha18_curves[[nm]], t='l', log='xy', ylim=EXRATE_PLOT_YLIM, col='skyblue', lwd=3,
            xlab='-Min-stage above MSL (m)', ylab='Exceedance-rate (events/year)', cex.lab=1.4, cex.axis=1.4)
        points(nonlinear_model_curves[[nm]]$min_stage - nonlinear_model_MSL,
        ...
        points(nonlinear_model_curves[[nm]]$min_stage - nonlinear_model_MSL,
               pmin(lower_CI, 1e-200), col='black', t='l', lty='dashed') # Lower bound for log-log plot
        points(nonlinear_model_curves[[nm]]$min_stage - nonlinear_model_MSL,
               pmin(upper_CI, 1e-200), col='black', t='l', lty='dashed') # Lower bound for log-log plot
    ...
    # Make a plot like the above, but replace the nonlinear model min-stages
    # with the PTHA18 min-stages (adjusted for MSL). This is useful for
    # testing, because with this approach, all differences in results are due
    # to Monte Carlo sampling.
    out_file = paste0(dir_name, '/Exceedance_Rates_in_PTHA18_and_Sampled_Scenarios_using_PTHA18_min_stages_', 
        target_point[1], '_', target_point[2], '.png')
    NPANEL = length(names(nonlinear_model_curves))
    HT = 7.5
    WD = HT * NPANEL
    ...
    out_file = paste0(dir_name, '/Exceedance_Rates_Summed_in_PTHA18_and_Nonlinear_Model_',
        target_point[1], '_', target_point[2], '.png')
    ...
    plot(ptha18_combined_curve, t='l', log='xy', ylim=EXRATE_PLOT_YLIM, col='skyblue', lwd=3,
        xlab='-Min-stage above MSL (m)', ylab='Exceedance-rate (events/year)', cex.lab=1.4, cex.axis=1.4)
    points(nonlinear_model_combined_curve$min_stage - nonlinear_model_MSL,
    ...
    # Plot like above, but replacing the nonlinear-model min-stages with the
    # PTHA18 min-stages (adjusted for MSL). This is useful for testing, because
    # with this approach, all differences in results are due to Monte Carlo
    out_file = paste0(dir_name, '/Exceedance_Rates_Summed_in_PTHA18_and_Sampled_scenarios_using_PTHA18_min_stages_',
        target_point[1], '_', target_point[2], '.png')
    png(out_file, width=HT, height=HT, units='in', res=300)
    options(scipen=5)
    plot(ptha18_combined_curve, t='l', log='xy', ylim=EXRATE_PLOT_YLIM, col='skyblue', lwd=3,
...
estimate_stage_exrate_curves_with_IS_for_all_logic_tree_branches<-function(
    ...
    for(i in 1:NLTB){
        ...
        #        stage_sampled_scenarios = sampled_scenarios_min_stage,
        ...
        for(j in 1:length(threshold_stage_values)){
            exrate_by_stage_threshold[j,i] = sum(
                sampled_scenario_rate * (sampled_scenarios_min_stage < threshold_stage_values[j])/
...
plot_epistemic_uncertainties_in_PTHA18_and_nonlinear_model_by_source_zone<-function(
    ...
    min_stage_type = 'nonlinear_model',
    dir_name = "."
    ){
    ...
    if(min_stage_type == 'nonlinear_model'){
        # Real case
        out_file = paste0(dir_name, '/Exceedance_Rates_with_epistemic_uncertainty_in_PTHA18_and_Nonlinear_Model_', 
            target_point[1], '_', target_point[2], '.png')
    }else if(min_stage_type == 'ptha18'){
        # Useful test case
        out_file = paste0(dir_name, '/Exceedance_Rates_with_epistemic_uncertainty_in_PTHA18_and_sampled_ptha18_scenarios_', 
            target_point[1], '_', target_point[2], '.png')
    }else{
        stop(paste0('unknown min_stage_type: ', min_stage_type))
    }
    ...
    for(source_zone in names(percentile_uncertainty_results)){
        ...
        if(min_stage_type == 'nonlinear_model'){
            mytitle = '-Min-stage exceedance-rate with epistemic uncertainty \n PTHA18 & nonlinear model: '
        }else if(min_stage_type == 'ptha18'){
...
plot_summed_exrates_with_epistemic_uncertainty_in_PTHA18_and_nonlinear_model<-function(
    ...
    if(min_stage_type == 'nonlinear_model'){
        # Real case
        out_file = paste0(dir_name, '/Exceedance_Rates_Summed_with_epistemic_uncertainty_in_PTHA18_and_Nonlinear_Model_', 
            target_point[1], '_', target_point[2], '.png')
    }else if(min_stage_type == 'ptha18'){
        # Useful test case
        out_file = paste0(dir_name, '/Exceedance_Rates_Summed_with_epistemic_uncertainty_in_PTHA18_and_sampled_ptha18_scenarios_', 
            target_point[1], '_', target_point[2], '.png')
    }else{
        stop(paste0('unknown min_stage_type: ', min_stage_type))
    }
    ...
    plot(percentile_uncertainty_results[[source_zone]]$threshold_stages - nonlinear_model_MSL, 
         ...
         xlab='Tsunami minima (m above ambient sea-level)', 
    ...
    if(min_stage_type == 'nonlinear_model'){
        # Real case
        mytitle = '-Min-stage exceedance-rate with epistemic uncertainties \n PTHA18 & nonlinear model, sum over source-zones'
    }else if(min_stage_type == 'ptha18'){
