...
# INPUTS
#
domain_indices_to_process = seq(1, 133)
raster_output_dir = './gladstone_arrival_time_min_and_scenario_average'
...
asfm = new.env()
source('../probabilistic_inundation/application_specific_file_metadata.R', local=asfm, chdir=TRUE)
...
for(i in 1:length(source_zones_metadata)){
    sz = names(source_zones_metadata)[i]
    output_dir_i = output_dir_sz[i] 
    system(paste0('gdalbuildvrt -resolution highest ', output_dir_i, '/minimum_arrival_time.vrt ', output_dir_i, '/minimum_arrival_time_domain_*.tif'))
    system(paste0('gdalbuildvrt -resolution highest ', output_dir_i, '/mean_arrival_time.vrt ', output_dir_i, '/mean_arrival_time_domain_*.tif'))
}
