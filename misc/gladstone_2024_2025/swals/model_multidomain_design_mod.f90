...
real(dp) ::  global_ll(2) = [100.0_dp, -79.0_dp]

! Length/width of multidomain in degrees lon,lat
real(dp) :: global_lw(2) = [220.0_dp, 68.0_dp + 79.0_dp]
...
real(dp) ::  final_time_full_runs = 36.0_dp * 3600.0_dp
...
! (beware possible slowdown if not parameter)
...
character(len=charlen) :: tidal_adjustment_file = ""
...
character(len=charlen) :: inverts_file_list = "../inverts/swals_invert_files.txt"
 
! Point gauges (where we store time-series outputs)
character(len=charlen) :: swals_point_gauge_file = "../gauges/locations.csv"
