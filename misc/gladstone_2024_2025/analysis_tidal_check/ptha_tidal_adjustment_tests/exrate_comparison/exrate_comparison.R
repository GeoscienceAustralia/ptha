...
sea_level = 1.459
ZOOM_PLOTS_REGIONS = readOGR(paste0('plot_windows/plot_windows_', sea_level), verbose=FALSE)

out_dir = paste0('./plots_', sea_level)
dir_combined=paste0("combined_figures_", sea_level)
...
##
TIF_FILES = list()
TIF_FILES$inundation_rate_ltm = Sys.glob('../../probabilistic_inundation/exrate_given_threshold/ptha_tidal_check/sea_level_vary/highres_depth_with_variance/sea_level_vary-depth-LogicTreeMean-sum_of_source_zones/summed_HS_domain_*_depth_as_max_stage_minus_elevation0_domain__exceedance_rate_with_threshold_0.001*.tif')
TIF_FILES$exrate_3knots_ltm = Sys.glob('../../probabilistic_inundation/exrate_given_threshold/ptha_tidal_check/sea_level_vary/highres_max_speed_with_variance/sea_level_vary-max_speed-LogicTreeMean-sum_of_source_zones/summed_HS_domain_*_max_speed_domain__exceedance_rate_with_threshold_1.543.tif')
# TIF_FILES$exrate_0knots_ltm = Sys.glob('../../probabilistic_inundation/exrate_given_threshold/ptha_tidal_check/sea_level_vary/highres_max_speed_with_variance/sea_level_vary-max_speed-LogicTreeMean-sum_of_source_zones/summed_HS_domain_*_max_speed_domain__exceedance_rate_with_threshold_0.001.tif')

TIF_FILES_STATIC = list()
TIF_FILES_STATIC$inundation_rate_ltm = Sys.glob(paste0('../../probabilistic_inundation/exrate_given_threshold/ptha_tidal_check/sea_level_', sea_level, '/highres_depth_with_variance/sea_level_', sea_level, '-depth-LogicTreeMean-sum_of_source_zones/summed_HS_domain_*_depth_as_max_stage_minus_elevation0_domain__exceedance_rate_with_threshold_0.001.tif'))
TIF_FILES_STATIC$exrate_3knots_ltm = Sys.glob(paste0('../../probabilistic_inundation/exrate_given_threshold/ptha_tidal_check/sea_level_', sea_level, '/highres_max_speed_with_variance/sea_level_', sea_level, '-max_speed-LogicTreeMean-sum_of_source_zones/summed_HS_domain_*_max_speed_domain__exceedance_rate_with_threshold_1.543.tif'))
# TIF_FILES_STATIC$exrate_0knots_ltm = Sys.glob(paste0('../../probabilistic_inundation/exrate_given_threshold/ptha_tidal_check/sea_level_', sea_level, '/highres_max_speed_with_variance/sea_level_', sea_level, '-max_speed-LogicTreeMean-sum_of_source_zones/summed_HS_domain_*_max_speed_domain__exceedance_rate_with_threshold_0.001.tif'))
...
INUNDATION_RATE_NAMES = c('inundation_rate_ltm', 'exrate_3knots_ltm')
...
get_osm_backdrop_reproj<-function(lower_left, upper_right, dbox, backdrop, zoom=14, cache_dir='.'){
    osm_cachefile = paste0(cache_dir, '/osm_tile_cache_', upper_right[1], upper_right[2], lower_left[1], lower_left[2], '.RDS')
    dmax = max(dbox)
...
#
regional_plot<-function(tif_files, lower_left, upper_right, backdrop='osm', plotvar='max_stage', colpal = pal1_trans,
    zlim=c(0, 10)){
    # Slightly reduce the LHS white-space margin. This was to reduce a gap
    # (whitespace) between combined figures
    orig_mar = par('mar')
    newmar = c(3.1, 2.0, 3.1, 1.5)
    par('mar' = newmar)
    ...
    zoom=15
    cache_dir = paste0('./osm_cache_zoom_', zoom)
    if(!file.exists(cache_dir)) dir.create(cache_dir)
    osm_backdrop_reproj = get_osm_backdrop_reproj(tmp_ll, tmp_ur, dbox, backdrop, zoom = zoom, cache_dir=cache_dir)
    ...
    rect(tmp_ll[1], tmp_ll[2], tmp_ur[1], tmp_ur[2], border=NA, col=rgb(1,1,1,alpha=0.2,maxColorValue=1))
    ...
    xy_lower = c(plot_ext@xmin + 0.6*(plot_ext@xmax-plot_ext@xmin), 
                plot_ext@ymin  + 0.1*(plot_ext@ymax-plot_ext@ymin))
...
make_region_plot<-function(i, out_dir='.', dir_combined='.'){
    ...
    inundation_ltm_file_static = paste0(out_dir, "/", siteName, '_inundation_rate_ltm_static.png')
    png(inundation_ltm_file_static, width=P_W, height=P_H, units='in', res=P_RES)
    regional_plot(TIF_FILES_STATIC, lower_left=plot_ll, upper_right=plot_ur, plotvar='inundation_rate_ltm', 
        zlim=c(NA, NA), backdrop='osm', colpal=NA)
    dev.off()
    
    # Plot inundation exceedance-rates, 3 knots.
    exrate_3knots_ltm_file_static = paste0(out_dir, "/", siteName, '_exrate_3knots_ltm_static.png')
    png(exrate_3knots_ltm_file_static, width=P_W, height=P_H, units='in', res=P_RES)
    # title(paste0(siteName, ': 3 knot exceedance rate \n logic tree mean'), cex.main=1.9)
    regional_plot(TIF_FILES_STATIC, lower_left=plot_ll, upper_right=plot_ur, plotvar='exrate_3knots_ltm', 
        zlim=c(NA, NA), backdrop='osm', colpal=NA)
    dev.off()

    # 1. Sea level vary
    # Plot inundation exceedance-rates, logic tree mean
    inundation_ltm_file_vary = paste0(out_dir, "/", siteName, '_inundation_rate_ltm_vary.png')
    png(inundation_ltm_file_vary, width=P_W, height=P_H, units='in', res=P_RES)
    regional_plot(TIF_FILES, lower_left=plot_ll, upper_right=plot_ur, plotvar='inundation_rate_ltm', 
        zlim=c(NA, NA), backdrop='osm', colpal=NA)
    dev.off()
    
    # Plot inundation exceedance-rates, 3 knots.
    exrate_3knots_ltm_file_vary = paste0(out_dir, "/", siteName, '_exrate_3knots_ltm_vary.png')
    png(exrate_3knots_ltm_file_vary, width=P_W, height=P_H, units='in', res=P_RES)
    # title(paste0(siteName, ': 3 knot exceedance rate \n logic tree mean'), cex.main=1.9)
    regional_plot(TIF_FILES, lower_left=plot_ll, upper_right=plot_ur, plotvar='exrate_3knots_ltm', 
        zlim=c(NA, NA), backdrop='osm', colpal=NA)
    dev.off()


    # Plot inundation exceedance-rates, 84th percentile
    # inundation_84pc_file = paste0(out_dir, "/", siteName, '_inundation_rate_84pc.png')
    ...
    # inundation_rate_legend_file = paste0(out_dir, "/", siteName, '_inundation_rate_legend.png')
    ...
    # images = image_read(c(inundation_ltm_file_static, inundation_ltm_file_vary, exrate_3knots_ltm_file_static, exrate_3knots_ltm_file_vary))
    # iminfo = image_info(images[1])
    # combined_figs = image_montage(images, tile="2x2", geometry=paste0(iminfo$width, 'x', iminfo$height))
    # image_write(combined_figs, path=paste0(out_dir, "/", siteName, '_combined.png'), format='png')
    ...
    # Function to create annotated plot
    create_annotated_plot <- function(image_path, col_title = NULL, row_title = NULL) {
    ...
    ggsave(filename = image_path, plot = plot)
    # return(plot)
    }

    # Create individual plots
    create_annotated_plot(inundation_ltm_file_static, "Static Tide", "Inundation rate")
    create_annotated_plot(inundation_ltm_file_vary, col_title = "Tidal Adjustment")
    create_annotated_plot(exrate_3knots_ltm_file_static, row_title = "3 knots exceedance rate")
    create_annotated_plot(exrate_3knots_ltm_file_vary)

    # Combine plots into a single plot
    # combined_plot <- plot_grid(
    # inundation_ltm_file_static_plot, inundation_ltm_file_vary_plot,
    # exrate_3knots_ltm_file_static_plot, exrate_3knots_ltm_file_vary_plot,
    # ncol = 2, align = 'v',
    # rel_widths = c(1, 1),
    # rel_heights = c(1, 1)
    # )
    # Save the combined plot
    # ggsave(filename = paste0(dir_combined, '/', siteName, '_combined.png'), plot = combined_plot)

    ## Combine into a single plot
    four_images = image_read(c(
        inundation_ltm_file_static,
        inundation_ltm_file_vary,
        exrate_3knots_ltm_file_static,
        exrate_3knots_ltm_file_vary)
    )
    ...
    image_write(combined_figs, path=paste0(dir_combined, '/', siteName, '_combined.png'), format='png')
...
dir.create(dir_combined)
for(i in my_inds) make_region_plot(i, out_dir=out_dir, dir_combined=dir_combined)
# num_cores=24
# mclapply(my_inds, function(i) make_region_plot(i, out_dir=out_dir), mc.cores = num_cores)
