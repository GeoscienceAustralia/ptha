...
template_qsub_file = "run_compute_exceedance_rates_at_epistemic_uncertainty_VARIABLE_SOURCEZONE_PERCENTILE_LOWER_UPPER_EXCEEDANCETHRESHOLD.pbs"
...
# A single variable
variable = 'max_stage'     # 'depth' or 'max_stage'
...
# Skip calculations for domain 1? Usually it is too big, and not of interest.
skip_domain_1 = TRUE
...
uppers = dbounds[-1]
if(skip_domain_1){
    # Deliberately skip domain 1 as it's too big
    lowers = dbounds[-n] + 1  
}else{
    # Include domain 1 (beware runtime blowout)
    lowers = dbounds[-n] + 1
    lowers[1] = 1
}
