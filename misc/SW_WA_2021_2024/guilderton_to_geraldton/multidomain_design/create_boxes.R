...
for(i in seq(length(nesting), 1, by=-1)){
    ...
    region_boundaries_combined = lapply(region_polys, function(x){ 
    ...
    region_boundaries_combined = do.call(rbind, region_boundaries_combined)

    # Only keep unique regions
    region_boundaries <- unique(region_boundaries_combined)
