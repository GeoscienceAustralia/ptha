...
all_marine_warning_tifs = Sys.glob('Inundation_zones/Perth-Coast/marine_warning_max_stage*.tif')
stopifnot(all(file.exists(all_marine_warning_tifs)))

# Elevation files for all the tifs
all_elevation_tifs = paste0(
    '../../swals/OUTPUTS/Fuji_andaman2004_24hrs_domain010322_timevaryingRealistic-full-ambient_sea_level_0.0/RUN_20220303_092730617/',
    gsub('marine_warning_max_stage_', 'elevation0_', basename(all_marine_warning_tifs)))
stopifnot(all(file.exists(all_elevation_tifs)))

# Write tifs here
output_dir = paste0(dirname(all_marine_warning_tifs[1]), '/Perth-Coast_marine_warning_depth_maxima')
