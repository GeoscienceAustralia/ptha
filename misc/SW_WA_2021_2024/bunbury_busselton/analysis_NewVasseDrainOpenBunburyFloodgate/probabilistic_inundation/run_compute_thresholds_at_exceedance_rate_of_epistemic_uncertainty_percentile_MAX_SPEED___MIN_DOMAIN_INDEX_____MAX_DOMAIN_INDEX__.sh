...
OUTPUTDIR=bunburyBusseltonNewVasseDrainOpenBunburyFloodgate_revised_highres_domains_max_speed_at_epistemic_uncertainty_84pc
...
for domain_index in $(seq $MIN_DOMAIN_INDEX $MAX_DOMAIN_INDEX); do
    Rscript compute_threshold_at_exceedance_rate_of_epistemic_uncertainty_percentile.R max_speed $domain_index $PERCENTILE $EXRATE $MINDEPTH $MAXDEPTH $DEPTHTOL $OUTPUTDIR
done
