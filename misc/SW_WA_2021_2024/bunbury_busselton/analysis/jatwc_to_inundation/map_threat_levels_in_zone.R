...
#ATWS_ZONE_NAME = 'Bunbury Geographe Coast'
...
get_rasters_touching_warning_zone<-function(
    ...
    warning_zone_buffer = st_buffer(warning_zone, dist=set_units(warning_zone_buffer_degrees, 'arc_degree')) # Buffer dist in degrees. Because we are using GEOS the units don't seem to matter.
    sf_use_s2(using_s2)
...
all_results = mclapply(gsub('max_stage', 'arrival_time', rasters_touching_warning_zone),
    parallel_fun_min, mc.cores=MC_CORES)
