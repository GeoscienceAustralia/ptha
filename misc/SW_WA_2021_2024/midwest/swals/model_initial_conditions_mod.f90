...
subroutine setup_elevation(domain, all_dx_md)
    ...
    if(smooth_elevation_along_nesting_boundaries) then
        ...
        ! Region near Yallingup, WA smoothed in model in redo 1, 2
        call domain%smooth_elevation_near_point(number_of_9pt_smooths = 1_ip, & ! num_smooth,
            pt = [114.998456790123_dp, -33.6663580246914_dp], smooth_region_radius_meters = 3000.0_dp, &
           transition_region_radius_meters = 3500.0_dp)
        ! Region near Darwin, WA smoothed in model in redo 3
        call domain%smooth_elevation_near_point(number_of_9pt_smooths = 1_ip, & ! num_smooth,
            pt = [131.4824_dp, -12.2875_dp], smooth_region_radius_meters = 5000.0_dp, &
           transition_region_radius_meters = 6000.0_dp)
        ! Additional region near Yallingup, WA brutally smoothed in model in redo 4
           call domain%smooth_elevation_near_point(number_of_9pt_smooths = 3_ip, & ! num_smooth,
           pt = [114.997_dp, -33.667_dp], smooth_region_radius_meters = 2000.0_dp, &
          transition_region_radius_meters = 2500.0_dp)
